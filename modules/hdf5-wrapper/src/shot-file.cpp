...
void ShotFile::close_file() { file.close(); }
...
void ShotFile::get_global_list_dict(std::string global_name,
    ...
    std::vector<LabscriptDictType> result;
    for (auto &item : list_dict_str) {
        result.push_back(ShotFile::convert_chars_to_labscript_dict(item));
    }
    *output = result;
}
...
std::vector<std::string> ShotFile::get_global_names() {
    ...
    auto iter_get_attribute_name =
        [](H5LocationCompat &loc, H5std_string attr_name, void *operator_data) {
    ...
    for (int i = 0; i < keyValuePairListSize; i++) {
        ...
        if (str_value.find('\'') != std::string::npos) {
            value = str_value.substr(1, str_value.length() - 2);
                   ...
                   str_value.find('E') != std::string::npos) {
            // Conversion of a string to double, taking into account that the
            // string could be in scientific format.
