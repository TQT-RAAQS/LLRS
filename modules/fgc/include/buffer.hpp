...
typedef struct gpuObjectDescRec {
    ...
    /* If buffer */
    uint32_t
        dwSize; /* Specifies the surface size if it's non renderable format */
} gpuObjectDesc;
...
class CBuffer {
  public:
     ...
     * @param [in] device Optional pointer to Direct3D device, should be NULL
     * when other GPU API used
     * @return
     */
    ...
    /**
     * @brief Initialise this object with the GPU object provided (allocates
     * system memory for DMA, semaphore, etc.).
     * @param desc [in]        Description of the GPU object
     * @param pGPUObject [in]  Pointer to the GPU object
     * @return etBufferStat    Error status
     */
    etBufferStat Init(gpuObjectDesc &desc, void *pvGPUObject,
                      int nBitDepth = 8);
    ...
    /**
     * Setup a DMA to transfer data to GPU and blocks until transfer completes.
     * The transfer starts when @TransferChunk() is called. Should be called
     * once for each PHX_INTRPT_BUFFER_READY event.
     * @return etBufferStat    Error status
     */
    etBufferStat GetFrame();

    /**
     * Trigger the DMA transfer that was setup by @GetFrame(). Should be called
     * for each chunk of the buffer to transfer; chunks are not currently
     * supported and it should be called for each PHX_INTRPT_BUFFER_READY event.
     * Call before \GetFrame() if both are called from the same thread.
     * @return etBufferStat    Error status
     */
    ...
    /**
     * Retrieve address of the buffer in system memory associated with the GPU
     * object.
     ...
     * @return void * Pointer to the GPU object; application should cast this to
     * appropriate GPU object type.
     */
    void *GetGPUObject();

    /**
     * Return stride of system memory buffer required by DVP library; this
     * should be used to set parameter PHX_BUF_DST_XLENGTH in the PHX library.
     * @return std::uint32_t The stride of the buffer
     */
    uint32_t GetBufferStride() const {
        return m_dwStride;
    } /* Returns the stride of the buffer in system memory */
    ...
    } /* Returns the size of the buffer in system memory */
  ...
  protected:
    ...
    void *
        m_pvSysMemBuffer; /* Aligned memory (same as sysMemAlloc for windows) */
    ...
    /* Initialises the stGPUMemInfo for the tech if not already done and returns
     * it. */
    static etBufferStat GetGPUMemInfo(etGPUAPI eGPUAPI, void *pD3DDevice,
                                      struct stGPUMemInfo **ppsGPUMemInfo);
};
