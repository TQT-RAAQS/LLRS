 ...
 *                           TMG_BAY_RGGB32_map_to_Y8()    Bayer to   Y8
 *converter, TMG_BAY_RGGB32_map_to_RGB24() Bayer to RGB24 converter,
 ...
 *   - 04-Jan-02, RCB, Added TMG_draw_get_ptr(), TMG_draw_text(),
 *TMG_IP_image_insert().
 *
 ...
 *   - 17-Jan-02, RCB, TMG_IP_image_insert() returns error if pixel formats are
 *different.
 ...
 *     colorkey overlay support in Windows (where overlay refers to incoming
 *video stream).
 *
 * v4.31 Rev.00, Jan 2008, HCP
 *   - Fixed some threadsafe issues in several functions. The simple way to
 *achieve this, at the expense of losing the ability to process in strips, is to
 *     remove the static settings on "local_action" and "lines_so_far".
 *     .\threadsafe.c maintains a list of what functions are threadsafe, plus
 *each function has a "--threadsafe--" comment if threadsafe.
 *   - TMG_BAY_ColorBalance() added.
 *   - Bayer (TMG_RGGB32 support added for TMG_LUT_ functions.
 *
 * v4.32 Rev.00, May 2008, HCP
 *   - Tweak to MJPEG recording to increase internal buffer for JPEG data (per
 *frame) from 200k to 1Mb.
 *
 * v4.33 Rev.00, 18-Jun-08, HCP
 *   - The following line added to TMG_image_check() to fill in the raw
 *"num_bytes_data": Pimage->num_bytes_data = Pimage->bytes_per_line *
 *Pimage->lines_this_strip;
 *
 * v4.34 Rev.00, 13-Jul-08, HCP
 *   - TMG_MRAW sequence recording and replay functions added for uncompressed
 *images. All contained in tmg_mraw.c.
 *
 * v4.35 Rev.00, 01-Oct-08, HCP
 *   - The fixed internal buffer of 1Mb for MJPEG recording is now dynamic to
 *cope with any size source data.  Y16 MRAW record and playback thoroughly
 *tested.
 *
 * v4.40, Rev.00 04-Oct-09, HCP
 *   - AVI support added for MJPEG.
 *   - DVR rebranding started.
 *   - 64 bit definitions added to tmg_os.h for Win32, Linux and Mac (from
 *Grahams AVI library).
 ...
 *   - 35% speed increase in JPEG compression from BGR24 by including the
 *conversion to YUV422 within the core JPEG algorithm.
 *   - Added direct TIFF file saving for BGR24.
 *
 * v4.42, Rev.00 26-Nov-09, HCP
 *   - Now BGRX32 included into core JPEG algorithm like BGR24 above.  Compared
 *to MVSC6, this new VS2005 is over double the speed for BGRX32!  (Note VS2008
 *appears 3% slower for JPEG compression.)
 *
 * v4.50, Rev.00 01-Jun-10, HCP
 *   - Some minor tidy-ups, following getting libraries sorted on PC with all
 *latest SDKs etc.
 *
 * v4.51, Rev.00 04-Oct-10, HCP
 *   - Modifications to MRAW reading so that it can cope with truncated files
 *(i.e. end corruption).
 *   - TMG_MJPEG_SetInfo() and TMG_MRAW_SetInfo() now has CriticalSection
 *serialization.
 ...
 *
 * v4.55, 06-Feb-12, HCP
 *   - Bug fix: Using LZW compression had side effect of converting original
 *image to RGB24. The problem was in tmg_api2.c, not in JP's LZW writer.
 *   - JPEG padding now works for YUV422 images.
 *
 * v4.56, 04-Apr-12, HCP
 *   - Latest (v4.1.1) AVI library included, which involved the addition of
 *frame rate and frame period.
 *
 * v4.57, 05-Apr-12, HCP
 ...
 *
 * v4.60, 19-Jun-12, HCP
 *   - TMG_IP_crop - Roi parameter had to be changed to i32 rather than ui16 to
 *cope with images > 65535 pixels wide or long.
 *   - New file added, tmg_histogram_stats.c containing new histogram and
 *statistics function, only supoprts Y8 so far.
 *   - TMG_RAW file write fixed to support single pixel wide images (i.e. saving
 *a column from an image).
 *
 * v4.62, 02-Jul-12, HCP
 *   - Y16, RGB24, BGR24 and BGRX32 formats now supported by
 *TMG_IP_Histogram_Statistics(). For 16 bit data, data assumed msb aligned, and
 *data_width determines width. Histogram and statistics functions fully support
 *10,12,14,16 bit.
 *   - All the convert _to_Y8 are now threadsafe.
 *   - TMG_IP_Subsample() completed, 32 bit pixel formats added, and threadsafe.
 *   - LFG licence checking now works for up to 4 LFG devices in separate
 *threads (previous only 1 thread worked).
 *
 *
 * v4.63, 23-Jul-12, HCP
 *   - New function TMG_IP_Subsample_by_Binning() added.
 *   - TMG_Image_Create() behaviour changed: 0 = no memory allocation, else now
 *in Bytes, not Pixels. (Also image buffer is TMG_LOCKED if any memory set or
 *allocated.)
 ...
 *   - TMG_Image_FileWrite() now compresses internally if JPEG requested and a
 *non-JPEG file used.
 *   - TIFF & BMP read/writers tested for working with different strides
 *(alignment). (JPEG read/write/compression does not support this - would need
 *to be copied first using TMG_ALIGNMENT_LINE_OUT to remove any (large) stride.)
 *   - TMG_IP_Convert() - significantly improved with all the usual formats
 *working with variable alignment and threadsafe.
 *   - TMG_IP_Crop() - Now supports variable alignment in addition to
 *threadsafe.
 *
 *
 * v4.64, 30-Aug-13, ED
 *   - Make TMG_IP_Image_Convert() preserve the TMG_USER_ALLOCATED_MEM status.
 *   - Make TMG_IP_Bayer_Convert_to_BGRX32() preserve the TMG_USER_ALLOCATED_MEM
 *status.
 *   - Remove change of TMG_LOCKED status in TMG_Image_ParameterGet().
 *
 *
 * v4.65, 19-Mar-14, ED
 *   - Make TMG_image_copy_parameters() and TMG_image_copy_parameters_raw()
 *correctly preserve the TMG_LOCKED status of the output image as well as the
 *TMG_USER_ALLOCATED_MEM status of the output image.
 *   - Increase input and output filename lengths.
 *   - Revert changes to preserving the TMG_USER_ALLOCATED_MEM status in
 *TMG_IP_Image_Convert() and TMG_IP_Bayer_Convert_to_BGRX32().
...
#define TMG_YUV422                                                             \
    ((ui16)12) /* 4:2:2 CCIR Digital Video at square pixel sampling */
...
#define TMG_RGGB32 ((ui16)16) /* Bayer encoded image, 8 bits per colour plane  \
                               */
#define TMG_RGGB64                                                             \
    ((ui16)17) /* Bayer encoded image, up to 16 bits per colour plane */
...
#define TMG_I32                                                                \
    ((ui16)100) /* for compression/computation - 1 x i32 per pixel - e.g Y8 */
#define TMG_I32x2                                                              \
    ((ui16)101) /* for compression/computation - 2 x i32 per pixel - e.g YUV   \
                 */
#define TMG_I32x2YUV411                                                        \
    ((ui16)102) /* special extension to recognise YUV411 sampling */
#define TMG_I16                                                                \
    ((ui16)103) /* for compression/computation - 1 x i16 per pixel - e.g Y8 */
#define TMG_I16x2                                                              \
    ((ui16)104) /* for compression/computation - 2 x i16 per pixel - e.g YUV   \
                 */
#define TMG_Y8_OR_RGB24                                                        \
    ((ui16)105) /* special case for paletted conversion (from paletted) */
#define TMG_YUV411 ((ui16)106) /* 4:1:1 (limited support - used in JPEG only)  \
                                */
#define TMG_YYUV                                                               \
    ((ui16)107) /* Special for JPEG decompression - YUV 4:2:2 with byte        \
                   ordering YYUV */
...
#define TMG_ALL_DATA_STRUCTURES                                                \
    ((Thandle)TMG_NUM_IMAGES + 2) /* unique identifier */
 ...
 * -----------------------------
 */
/* 3-Mar-08: Request for full transparency added plus a couple of others and
 * keeping compatible */
#define TMG_DRAW_BG_OPAQUE 1 /* Gray background  at 127 */
#define TMG_DRAW_BG_TRANSPARENT_0                                              \
    2 /* 0% transparent,   BG = Background: black  */
#define TMG_DRAW_BG_TRANSPARENT_50                                             \
    3 /* 50% transparent,  BG = BackGround: 0..127 */
#define TMG_DRAW_BG_TRANSPARENT_100                                            \
    4 /* 100% transparent, BG = BackGround, 0..255 */
...
/*#define TMG_TARGA        5    Some type of Targa file. Code stripped out,
 * 5-Aug-09 */
...
struct tTMG {
    ...
    ui32 dwOtherLibErrCode; /* To record error if from different library (e.g.
                               driver) */
...
#define TMG_LINE_ALIGNMENT_DEFAULT                                             \
    ((ui32)2)            /* NOTE: alignment 16 bit for now */
...
#define TMG_USER_ALLOCATED_MEM                                                 \
    ((ui32)0x100) /* Indicates memory buffer is by user */
#define TMG_JPEG_DO_NOT_WRITE_HUFFMAN_TABLE                                    \
    ((ui32)0x200)                  /* Save space and time in AVI MJPEG */
#define TMG_TIFF_LZW ((ui32)0x400) /* JPA: Enables LZW for TIFF */
#define TMG_TIFF_LZW_PREDICTOR                                                 \
    ((ui32)0x800) /* JPA: Enables LZW predictor for TIFF */
...
struct Ttmg_LUT {
    ...
    ui8 *green_ui8;
    ui8 *blue_ui8;
    IM_UI16 *luma_ui16; /* Must be _huge as can have 65536 elements (of 2 bytes
                           each) */
    IM_UI16 *red_ui16;
    IM_UI16 *green_ui16;
    IM_UI16 *blue_ui16;
...
struct sTMG_Font {
    ...
    ui32 dwBytesPerChar; /* Number of bytes defining each character in font
                            table */
...
#define TMG_YUV_TO_RGB16_LUT_SIZE                                              \
    ((ui32)1 << (TMG_Y_BITS16 + TMG_U_BITS16 + TMG_V_BITS16))
#define TMG_YUV_TO_PALETTED_LUT_SIZE                                           \
    ((ui32)1 << (TMG_Y_BITS8 + TMG_U_BITS8 + TMG_V_BITS8))
...
#define TMG_JPEG_Q_FACTOR_SCALING                                              \
    65536 /* Power(2, TMG_JPEG_Q_FACTOR_SCALING_BITS) */
#define TMG_JPEG_Q_FACTOR_SCALING_BITS                                         \
    16 /* Matched to TMG_JPEG_Q_FACTOR_SCALING     */
#define TMG_JPEG_USER_COMMENT_LEN                                              \
    32                          /* 32 bytes for user info in comment field  */
...
struct Tjpeg {
    ...
    ui32 num_bytes_data; /* amount of compressed data - not including EOI (FF
                            D9) */
    ...
    char szUserComment[TMG_JPEG_USER_COMMENT_LEN]; /* For user info - pre-pended
                                                      before ASL info */
...
#define TMG_AUTO_HEIGHT                                                        \
    ((i16)32767) /* bigger than any image - but still an i16 */
...
#define TMG_AUTO_CENTRE ((ui16)32766) /* auto-centre under DOS (only) */
    ...
    ui16 y_end;                       /* ROI */
#define TMG_ROI_INIT                                                           \
    ((ui16)0) /* set the display ROI to display a full image, positioned top   \
                 left */
#define TMG_ROI_SET ((ui16)1) /* set an ROI */
    ...
    unsigned long
        XPixelLUT[TMG_COLOURS]; /* Mapping our 'colours' to pixel values   */
    ...
    PixMapHandle
        hpmImg; /* Handle to pixmap describing this image - for CopyBits */
    ...
    CGLContextObj
        Context; /* The Apple GL Context associated with this display */
    Thandle hDisplayImage; /* Temporary handle used if an image conversion id
                              required */
#endif
...
#define TMG_MJPEG_JPEG_USER_COMMENT_LEN                                        \
    128 /* For user comments/spare, currently unused */
#define TMG_MJPEG_GPS_RMC_LEN 128 /* GPS RMC string */
#define TMG_MJPEG_COMMENT_LEN                                                  \
    (TMG_MJPEG_JPEG_USER_COMMENT_LEN + TMG_MJPEG_GPS_RMC_LEN +                 \
     40) /* Frame number, timestamp etc */
#define TMG_MJPEG_WRAPPER_OVERHEAD                                             \
    (2 + 2 + TMG_MJPEG_COMMENT_LEN + 2 +                                       \
     2) /* SETTING: COM+field_length, commentstring, SOS, EOI */
#define TMG_MJPEG_X1FWD_OFFSET                                                 \
    ((4 * 4) + 4) /* SETTING: Offset to x1rev parameter */
#define TMG_MJPEG_X1REV_OFFSET                                                 \
    ((7 * 4) + 4) /* SETTING: Offset to x1fwd parameter */
#define TMG_MJPEG_NUM_BYTES_JPEG_DATA_OFFSET                                   \
    ((1 * 4) + 4) /* SETTING: Offset to dwNumBytesJpegData parameter */
...
struct tTMG_MJPEG {
    ...
    struct tTMG_MJPEG
        *psIntMjpeg; /* Forward pointer - can be used by user to write to */
    ...
    ui8 szCommentString[TMG_MJPEG_COMMENT_LEN]; /* Contains frame number etc -
                                                   see tmg_mjpg.c */
    ...
    i32 fTimestamp_is_UserSet; /* Indicates that the application is filling in
                                  the dwTimestampSecs parameter */
    i32 fInUse;
...
struct tTMG_MRAW {
    ui32 hHandle; /* Handle to this MJPEG stream */
    struct tTMG_MRAW
        *psIntMraw; /* Forward pointer - can be used by user to write to */
...
_TMG_LengthOf()
*/
#define _TMG_GetAndCheckpsImage(s, Pi, Hi)                                     \
    {                                                                          \
        if ((Pi = Pimage_array[Hi]) == NULL)                                   \
            TMG_Roe(TMG_ERROR_BAD_HANDLE,                                      \
                    "TMG handle references null pointer", s)                   \
    }

#define _TMG_Limit_to_0(x)                                                     \
    {                                                                          \
        if ((x) < 0) {                                                         \
            (x) = 0;                                                           \
        }                                                                      \
    }
#define _TMG_Limit_to_255(x)                                                   \
    {                                                                          \
        if ((x) > 255) {                                                       \
            (x) = 255;                                                         \
        }                                                                      \
    }
#define _TMG_Limit_0_to_255(x)                                                 \
    {                                                                          \
        if ((x) < 0) {                                                         \
            (x) = 0;                                                           \
        } else if ((x) > 255) {                                                \
