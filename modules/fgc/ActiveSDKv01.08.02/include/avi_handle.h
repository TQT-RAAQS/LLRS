...
#define _ASL_BMP_CHUNK                                                         \
    FCC("01db") /* Video uncompressed bitmap data chunk type */
...
#define _ASL_CHUNK_LIMIT                                                       \
    8192 /* Maximum number of movi data chunks, each of which is up to roughly \
            _ASL_CHUNK_SIZE (1GB) in size & has one index.*/
#define _ASL_CHUNK_SIZE                                                        \
    1073741312 /* 1GB limit (rather less in fact, to be on the safe side), for \
                  any movi chunk. This is necssary because of restrictions in  \
                  many players. */
#define _ASL_CHUNK_FRAMES                                                      \
    20000 /* Maximum number of frames within any given chunk */
#define _ASL_BUFFER_FRAMES                                                     \
    100000 /* Maximum number of individual frames that can be written to RAM   \
              buffer */
#define _ASL_MIN_BUFFER_SIZE                                                   \
    104857600 /* Minimum size of memory buffer, excluding frame index, for     \
                 buffered writes */
#define _ASL_INIT                                                              \
    542390716 /* Magic number which when set in a global tells other functions \
                 that the function has been initialiazed */
#define _ASL_MJPG                                                              \
    "MJPEG" /* Strings hidden in the odml section at the end of the main       \
               header to define extended ASL types such as 48 bit */
...
#define _AVI_INVALID_ARGUMENT(Err)                                             \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Invalid Function Argument", AVI_INVALID_ARGUMENT); \
    }                                                                          \
    Err = AVI_INVALID_ARGUMENT;                                                \
    goto Exit
#define _AVI_INVALID_HANDLE(Err)                                               \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Invalid Handle", AVI_INVALID_HANDLE);              \
    }                                                                          \
    Err = AVI_INVALID_HANDLE;                                                  \
    goto Exit
#define _AVI_NOT_INIT(Err)                                                     \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Avi Library Not Initialized", AVI_NOT_INIT);       \
    }                                                                          \
    Err = AVI_NOT_INIT;                                                        \
    goto Exit
#define _AVI_NULL_POINTER(Err)                                                 \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Null Pointer Passed to Function",                  \
                           AVI_NULL_POINTER);                                  \
    }                                                                          \
    Err = AVI_NULL_POINTER;                                                    \
    goto Exit
#define _AVI_MEM_ALLOC(Err)                                                    \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Memory Allocation Error", AVI_MEM_ALLOC);          \
    }                                                                          \
    Err = AVI_MEM_ALLOC;                                                       \
    goto Exit
#define _AVI_MEM_REALLOC(Err)                                                  \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Memory Re-allocation Error", AVI_MEM_REALLOC);     \
    }                                                                          \
    Err = AVI_MEM_REALLOC;                                                     \
    goto Exit
#define _AVI_MEM_BUFFER_FULL(Err)                                              \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Memory Buffer Full", AVI_MEM_BUFFER_FULL);         \
    }                                                                          \
    Err = AVI_MEM_BUFFER_FULL;                                                 \
    goto Exit
#define _AVI_OUT_FILE_OPEN(Err)                                                \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Output File Open Error", AVI_OUT_FILE_OPEN);       \
    }                                                                          \
    Err = AVI_OUT_FILE_OPEN;                                                   \
    goto Exit
#define _AVI_HANDLE_IN_USE(Err)                                                \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Handle Already In Use", AVI_HANDLE_IN_USE);        \
    }                                                                          \
    Err = AVI_HANDLE_IN_USE;                                                   \
    goto Exit
#define _AVI_FILE_NAME(Err)                                                    \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Invalid File Name", AVI_FILE_NAME);                \
    }                                                                          \
    Err = AVI_FILE_NAME;                                                       \
    goto Exit
#define _AVI_WRITE_FAIL(Err)                                                   \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("File Write Error", AVI_WRITE_FAIL);                \
    }                                                                          \
    Err = AVI_WRITE_FAIL;                                                      \
    goto Exit
#define _AVI_IN_FILE_OPEN(Err)                                                 \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Input File Open Error", AVI_IN_FILE_OPEN);         \
    }                                                                          \
    Err = AVI_IN_FILE_OPEN;                                                    \
    goto Exit
#define _AVI_MAX_OPEN_FILES(Err)                                               \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Number of Open Files Exceeded",                    \
                           AVI_MAX_OPEN_FILES);                                \
    }                                                                          \
    Err = AVI_MAX_OPEN_FILES;                                                  \
    goto Exit
#define _AVI_READ_FAIL(Err)                                                    \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("File Read Error", AVI_READ_FAIL);                  \
    }                                                                          \
    Err = AVI_READ_FAIL;                                                       \
    goto Exit
#define _AVI_CORRUPT_FILE(Err)                                                 \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Input File Corrupt", AVI_CORRUPT_FILE);            \
    }                                                                          \
    Err = AVI_CORRUPT_FILE;                                                    \
    goto Exit
#define _AVI_FILE_RW(Err)                                                      \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Read / Write Mismatch", AVI_FILE_RW);              \
    }                                                                          \
    Err = AVI_FILE_RW;                                                         \
    goto Exit
#define _AVI_FILE_NOT_SUPPORTED(Err)                                           \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Unsupported Codec or File Type",                   \
                           AVI_FILE_NOT_SUPPORTED);                            \
    }                                                                          \
    Err = AVI_FILE_NOT_SUPPORTED;                                              \
    goto Exit
#define _AVI_END_OF_SEQUENCE(Err)                                              \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Read Beyond End of Sequence",                      \
                           AVI_END_OF_SEQUENCE);                               \
    }                                                                          \
    Err = AVI_END_OF_SEQUENCE;                                                 \
    goto Exit
#define _AVI_DATA_ERROR(Err)                                                   \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Image Data Error", AVI_DATA_ERROR);                \
    }                                                                          \
    Err = AVI_DATA_ERROR;                                                      \
    goto Exit
#define _AVI_THREAD_ERROR(Err)                                                 \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Multi-Threading Error", AVI_THREAD_ERROR);         \
    }                                                                          \
    Err = AVI_THREAD_ERROR;                                                    \
    goto Exit
#define _AVI_THREAD_CS_ERROR(Err)                                              \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Thread Critical Section Error",                    \
                           AVI_THREAD_CS_ERROR);                               \
    }                                                                          \
    Err = AVI_THREAD_CS_ERROR;                                                 \
    goto Exit
#define _AVI_AUDIO_ERROR(Err)                                                  \
    if (NULL != gfnAVIErrorHandler) {                                          \
        gfnAVIErrorHandler("Invalid audio parameters", AVI_AUDIO_ERROR);       \
    }                                                                          \
    Err = AVI_AUDIO_ERROR;                                                     \
    goto Exit
/* Other macros */
#define _AVI_FreeAndNull(pointer)                                              \
    if (NULL != pointer) {                                                     \
        free(pointer);                                                         \
    }

/* Version 1.3.1, pHandleData->dwMoviChunkSize onwards added */
/* Version 1.4, pbFrameImageBuffer and pbFrameAudioBuffer initialised.
 * pbInterlaceBuffer added and initialized */
#define _AVI_InitPrivateMembers                                                \
    pHandleData->fFile = -1;                                                   \
...
#define _AVI_InitPublicMembers                                                 \
    phFileHandle->pszFileBufferName = NULL;                                    \
...
#define _AVI_RefreshPublicMembers                                              \
    phFileHandle->pszFileBufferName = pHandleData->pszFileBufferName;          \
...
/* Portable version of Microsoft Four Character Code (FOURCC) type and type
 * conversion macro */

#define FOURCC ui32

/* Portable version of Microsoft BITMAPINFOHEADER. Uses unsigned types: none of
 * these values will ever be negative */
...
typedef struct tagBitMapInfoHeader {
    ...
    ui32 dwClrUsed;
    ui32 dwClrImportant;
    /*ui32   dwOffset;          Offset to JPEG specific fields. Optional
     * parameter seems to be unused by any player*/
} BitMapInfoHeader;
...
/* Portable version of Microsoft AVIPALCHANGE palette change structure for use
 * in video streams */
...
typedef struct tagJpegInfoHeader { /* Optional data structure. Seems to be
                                      unused by any player */
    ui32 dwJpegSize;
    ui32 dwJpegProcess;
...
typedef struct tagWaveFormatEx {
    ...
    ui16 wBlockAlign;
    ui16 wBitsPerSample;
    ui16 wExtendedSize; /* Size, in bytes, of extra format information appended
                           to the end of */
} WaveFormatEx; /* the WAVEFORMATEX structure. Must be set to 0 for unformatted
                   data. */
...
typedef struct tagSuperindex_Entry { /* 16 bytes */
    off64_t qfpOffset; /* 64 bit offset to sub index chunk, i.e. to start of
                          marker 'ix00' */
    ui32 dwSize;       /* 32 bit size of sub index chunk       */
    ui32 dwDuration;   /* time span of subindex chunk (in stream ticks). In this
                          implementation this is the number of */
...
typedef struct tagStandardIndex_Header { /* 32 bytes */
    ...
    off64_t
        qfpBaseOffset; /* base offset that all index intries are relative to */
    ui32 dwReserved_3; /* must be 0 */
} StandardIndex_Header;

typedef struct tagStandardindex_Entry { /* 8 bytes */
    ui32 dwOffset; /* 32 bit offset to data (points to data, not riff header,
                      e.g. to start of '00db' marker) */
    ui32
        dwSize; /* 31 bit size of data (does not include size of riff header) */
} StandardIndex_Entry; /* Bit 31 is deltaframe bit, always 0 for MJPEG files */

typedef struct tagASL_Index_Entry {
    off64_t qfpOffset; /* 64 bit Offset of start of data section from beginning
                          of file */
} ASL_Index_Entry;

typedef struct tagAviHandleData {
    /* The following are "private" data members only accessible through the void
     * *pReserved pointer in the file handle - if you know that this pointer
     * points to an AviHandleData structure!
     */
    AviHandle *phHandle;
    ...
    ui32 dwNumberOfSuperIndexes; /* Number of super indexes in use. Shouldn't
                                    have more than three (video, audio and text)
                                  */
    ui32 dwNumberOfVideoChunks;  /* Synonomous with the number of indexes in the
                                    super index structure for ASL generated files
                                  */
    ...
    ui32 dwAudioBufferSize; /* Suggested buffer size for audio - no longer fixed
                               size from version 1.4. onwards to cope with third
                               party files */
    ui32 dwMoviChunkSize;   /* Size of current movi chunk */
    ui32 dwOffset;          /* Offset in current movi chunk */
    ui32 dwAudioLength; /* Length of entire (audio stream of) file in units of
                           audio samples. */
    off64_t fpStartOfHeader; /* Start of a chunks' header */
    off64_t qfpBaseOffset;   /* This is the offset to the start of the chunk
                                index, as recorded in the super index */
    off64_t qfpIndexOffset;  /* This is the offset to the start of the first
                                frame, as recorded in the chunk index */
    ui32 dwFirstChunkSize;
    ui8 *pbDataChunkHeaderBuffer; /* Points to memory allocated once only to
                                     buffer the chunk header writes */
    ui8 *pbFrameImageBuffer; /* Single frame buffer used in "unbuffered" writes,
                                i.e. writes where the entire video isn't
                                buffered to memory */
    ui8 *pbFrameAudioBuffer; /* Likewise for audio */
    ui8 *pbInterlaceBuffer;  /* Buffer used to merge two fields in an interlaced
                                MJPEG file */
    ui32 dwAudioSamplesPerRecord; /* 1 for all files written using this library.
                                     Third party libraries may bundle lots of
                                     audio samples (frames) in one record */
    ui8 *pbMainHeaderBuffer; /* Points to memory allocated to buffer main header
                                reads */

    /* Used only when writing to memory */

    ui8 *pbMemoryFileBuffer;   /* Pointer to malloced area used to write / read
                                  successive frame information.  */
    ui8 *pbMemoryFramePointer; /* Points to actual frame data in the malloced
                                  area */
    ui64 *
        pqwIndexPointer; /* Points to index at the start of the malloced area */
    ...
    /* The following are duplicates of the public data members, held so that the
       user cannot (easily) modify important parameters and cause problems.
     */
    ...
    eStore_Type eStoreType; /* Specifies whether the file is in volatile memory
                               or on disk */
} AviHandleData;
...
/* These frame write functions are kept private. The library works out which one
 * to call based on whether the file has been opened for buffered output or
 * write to disk.
 */

AVI_ERR AVI_BufferedWriteNextFrame(AviHandle *phFileHandle, ui8 *pbBuffer,
                                   ui32 dwFrameSize, ui8 *pbAudioBuffer);

AVI_ERR AVI_UnBufferedWriteNextFrame(AviHandle *phFileHandle, ui8 *pbBuffer,
                                     ui32 dwFrameSize, ui8 *pbAudioBuffer);

/* Private functions to parse the video and audio super indexes.
 */

AVI_ERR AVI_ParseAudioSuperIndex(AviHandle *phFileHandle, fDesc iFile,
                                 ui8 *pbAudioBuffer,
                                 AVI_Main_Header sAviMainHeader);
AVI_ERR AVI_ParseVideoSuperIndex(AviHandle *phFileHandle, fDesc iFile,
                                 AVI_Main_Header sAviMainHeader,
                                 Stream_Header sStreamHeader);

/* Private functions to check for the presence of a valid "ix", "db", "dc" and
 * "wb" markers
 */
