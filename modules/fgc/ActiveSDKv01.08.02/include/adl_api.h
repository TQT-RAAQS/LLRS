...
typedef enum {
    /*! ADL will attempt to use Direct3D but will default to GDI if the hardware
     *does not ! support Direct3D.
     *!*/
    ADL_GRAPHICS_AUTO = 0,
...
typedef enum {
     ...
     *!      Default value.
     *! - ADL_ENABLE: Full screen mode is enabled, the display window has no
     *border !      and takes all available screen space. ! Argument type:
     *uin32_t *.
     */
    ADL_DISPLAY_INIT_FULL_SCREEN,
    /*! Enable VSync for rendering.
     *! Possible values:
     *! - ADL_DISABLE: VSync disabled. Display will render images as fast as
     *possible, regardless of VSync. ! - ADL_ENABLE: VSync enabled. Display will
     *render images in sync with VSync. Default value. ! Argument type: uin32_t
     **.
     */
    ...
    /*! Enable fit-to-display function, whereby the library resizes the image so
     *that it fits within ! the display window. ! When enabled, the zoom factor
     *is ignored. ! Related features: ADL_DISPLAY_KEEP_ASPECT_RATIO and
     *ADL_DISPLAY_SAMPLING_METHOD. ! Possible values are: ! - ADL_DISABLE:
     *Disabled. Default value. ! - ADL_ENABLE: Enabled. ! Argument type: uin32_t
     **.
     */
    ADL_DISPLAY_FIT_TO_DISPLAY,
    /*! Specify a point in the image that will remain at the same position in
     *the display ! when the image is zoomed. ! Argument type: ADL_Point *.
     */
     ...
     */
    ADL_DISPLAY_FORMAT,
    /*! Offset to apply to the image AOI currently displayed. Offset used for
     *the next rendering ! operation. ! Argument type: ADL_Dimensions *.
     */
     ...
     *! Only valid when ADL_DISPLAY_FIT_TO_DISPLAY = 1 and
     *ADL_DISPLAY_SAMPLING_METHOD = 0. ! Argument type: uin32_t *. ! Read-only.
     */
    ADL_DISPLAY_FIT_TO_DISPLAY_XY_ZOOM_FACTOR,
    /*! Allow synchronizing the scrollbars of another display with this one.
     *! When enabled, moving the scrollbars of one display also moves the
     *scrollbars of the other. ! The argument is the handle of the display to be
     *synchronized. ! Argument type: tADLDisplay *.
     */
    ADL_DISPLAY_SYNCED,
    /*! Control if the image aspect ratio should be preserved when the image is
     *over- or sub-sampled ! when fit-to-display is enabled. ! Only used when
     *ADL_DISPLAY_FIT_TO_DISPLAY = 1 and ADL_DISPLAY_SAMPLING_METHOD = 1; no
     *effect otherwise. ! Possible values: ! - ADL_DISABLE: Do not preserve
     *aspect ratio. ! - ADL_ENABLE: Preserve aspect ratio. Default value. !
     *Argument type: uin32_t *.
     */
    ADL_DISPLAY_KEEP_ASPECT_RATIO,
    /*! The pixel sampling method used when the image is over- or sub-sampled
     *when fit-to-display is enabled. ! Only used when
     *ADL_DISPLAY_FIT_TO_DISPLAY = 1; no effect otherwise. ! Possible values
     *are: ! - 0: Default value. !      The same sampling factor is used
     *horizontally and vertically. !      The image resolution may be reduced or
     *increased by integral factors only (-3, -2, 2, 3, 4, etc.). !      The
     *sampling either drops or duplicates pixels. No interpolation is performed.
     *!      Depending on the combination of image and display resolution, this
     *may result in parts of the !      display being not filled with image
     *data. ! - 1: Non-integral image sampling; the sampling factor is
     *calculated so that the image exactly fits the display. !      The
     *resulting sampling factor is non-integral. !      Note, when this option
     *is enabled and the Direct3D renderer is used, the image has to fit within
     *a Direct3D !      texture. The maximum resolution of such a texture can be
     *queried via ADL_DISPLAY_D3D_MAX_TEXTURE_RES. ! Argument type: uin32_t *.
     */
    ADL_DISPLAY_SAMPLING_METHOD,
    /*! Return the maximum width and height of a Direct3D texture for the
     *current graphics adapter. Library must have been ! initialized with
     *ADL_GRAPHICS_D3D, otherwise an error is generated. ! Argument type:
     *ADL_Dimensions *. ! Read-only.
     *!*/
 ...
 */
typedef enum {
    /*! Pointer to the buffer containing the image. Note, the image data must be
     *consecutive in the buffer, ! line padding is not supported. ! Argument
     *type: void *.
     */
    ADL_IMAGE_ADDRESS = 1,
    /*! Image width.
 ...
 * To be called once, prior to any function call to allow global initialization
 * of the library. \param eGraphicsLibrary The graphics library to use for the
 * rendering. Possible values: See etADLGraphicsLibrary declaration.
 */
etADLStat ADL_EXPORT ADL_InitLib(etADLGraphicsLibrary eGraphicsLibrary);

/*!
 * \brief Must be called after no function of the library is needed anymore to
 * free the resources from the ADL_InitLib() function call. Releases and
 ...
 * \param eGraphicsLibrary Contains the underlying rendering library used by
 * ADL. \return ADL_ERROR_LIBRARY_NOT_INITIALIZED if the library has not been
 * initialised yet with ADL_InitLib(). ADL_ERROR_INVALID_POINTER if a null
 * pointer is passed for \eGraphicsLibrary. ADL_OK otherwise.
 */
 ...
 * ADL_DisplayInit() must be called to initialize the object prior to any other
 * function.
 */
 ...
 * \param pvParamValue Pointer to a variable holding the value for the
 * parameter. See each parameter's documentation for the variable's type to use.
 */
etADLStat ADL_EXPORT ADL_DisplayParameterSet(tADLDisplay hDisplay,
                                             etADLDisplayParam eDisplayParam,
                                             void *pvParamValue);
 ...
 * \param pvParamValue Pointer to the variable that will receive the value of
 * the parameter. See each parameter's documentation for the variable's type to
 * use.
 */
etADLStat ADL_EXPORT ADL_DisplayParameterGet(tADLDisplay hDisplay,
                                             etADLDisplayParam eDisplayParam,
                                             void *pvParamValue);

/*!
 * \brief Select a new ADL image to be displayed.
 * The image will be processed as soon as the display has completed rendering of
 * the current image. The display does not copy the image internally, so it must
 * remain valid until the following call to ADL_DisplayImageSet(). \param
 * hDisplay Handle to the display. \param hImage The new image to display.
 * \param phPreviousImage The previous image that was selected and that is no
 * longer used by the display.
 */
etADLStat ADL_EXPORT ADL_DisplayImageSet(tADLDisplay hDisplay, tADLImage hImage,
                                         tADLImage *phPreviousImage);

/*!
 * \brief Try to lock the display. Returns immediately. On successful lock
 * acquisition, the display no longer updates and nor uses the current
 * tADLImage. \param hDisplay Handle to the display. \param Locked Null if
 * display not locked, non-null otherwise. \return etADLStat ADL_OK if no error.
 */
etADLStat ADL_EXPORT ADL_TryLock(tADLDisplay hDisplay, int *Locked);

/*!
 * \brief Unlock the display. The display must be locked by the current thread
 * of execution (via ADL_TryLock()), otherwise, the behavior is undefined.
 * \param hDisplay Handle to the display.
...
/*
 * \brief Create an ADL image object. The tADLImage object is a self-describing
 * container for an image. It should be initialized using the
 * ADL_ImageParameterSet(). \param phImage Pointer to the created image
 * instance.
 */
 ...
 */
etADLStat ADL_EXPORT ADL_ImageParameterSet(tADLImage hImage,
                                           etADLImageParam eImageParam,
                                           void *pvParamValue);
 ...
 * \param hImage Handle to the image.
 * \param eImageParam The parameter to set.
 * \param pvParamValue Pointer to a variable that will receive the value of the
 * parameter. See each parameter's documentation for the variable's type to use.
 */
etADLStat ADL_EXPORT ADL_ImageParameterGet(tADLImage hImage,
                                           etADLImageParam eImageParam,
                                           void *pvParamValue);
 ...
 * \param pszErrorMessage User-allocated array of characters that will contain a
 copy of the last error message.
 * \param pnSize [in] Pointer to the size in bytes of pszErrorMessage.
 [out] Pointer to the actual size in bytes of the error message copied to
 pszErrorMessage.
 */
etADLStat ADL_EXPORT ADL_GetLastError(etADLStat *peStat, char *pszErrorMessage,
                                      size_t *pnSize);
}
