...
#ifdef _PHX_WIN
#pragma warning( disable : 4032 )   /* warning C4032: formal parameter 1 has different type when promoted */
#pragma warning( disable : 4055 )   /* warning C4055: type cast from data pointer to function pointer */
...
#pragma warning( error   : 4001 )   /* Warning C4001: C++ comment is now an error; but this is only displayed
                                     * if the /Za option ("disabled Microsoft extensions") is enabled which then
                                     * generates HUGE numbers of errors and warnings in "windows.h".  It may be
                                     * possible not to include this header file in the librarues, but the CDA
                                     * library would also have to be fixed as this is included for the CDA_Buffer
                                     * info.
                                     */
#endif
 ...
 */
#ifdef __cplusplus
   #define _PHX_ErrPrint( szMessage, szTitle ) ::MessageBox( NULL, szMessage, szTitle, MB_OK );
#else
   #define _PHX_ErrPrint( szMessage, szTitle ) MessageBox( NULL, szMessage, szTitle, MB_OK );
#endif
...
#ifdef _PHX_DEBUG
   #define _PHX_ErrorTrap( _f, _s ) _PHX_Assert( _f ); if ( !(_f) ) _PHX_GotoErr( PHX_ERROR_INTERNAL_ERROR, _s );
#else
   #define _PHX_ErrorTrap( _f, _s ) if ( !(_f) ) _PHX_GotoErr( PHX_ERROR_INTERNAL_ERROR, _s );
#endif
...
#define _PHX_DebugTrace( _str ) { printf( "%s", _str ); }
 ...
 * ------------------------------------------------------
 */
#define _PHX_ErrPrint( szMessage, szTitle ) {printf( "\nERROR - %s: %s\n", szTitle, szMessage );}
...
#define _PHX_SleepMs(x) { \
   ...
   tReq.tv_sec  =  (x)/1000L;  tReq.tv_nsec = ((x)%1000L) * 1000000UL; \
...
#define _PHX_SleepMs(x) { \
   struct timespec tReq; \
   tReq.tv_sec  =  (x)/1000;  tReq.tv_nsec = ((x)%1000) * 1000000; \
   (void) nanosleep( &tReq, NULL); \
 }
