 ...
 * ----------------
 */
#define TMG_DEFAULT_WIDTH                                                      \
    2048 /* Image width and height, if no buffer size declared - see           \
            TMG_ImageCreate() */
...
struct tTMG_DVR {
    int fInUse;
    int fOpenedForInput; /* Flag to indicate whether opened for input (read) or
                            output (write) */

    int hHandle; /* Back reference to user handle number to reference instance
                    of structure */
    ...
    ui32 hJpegImage;
#define TMG_DVR_JPEG_BUFFER_SIZE                                               \
    4194304 /* SETTING: JPEG buffer size for recording */
    int nJpegBufferSize;
    ui8 *pbJpegBuffer;

#define TMG_DVR_AUDIO_BUFFER_SIZE                                              \
    (44100 * 2 *                                                               \
     2) /* 1 second of max sampling at 16 bits per channel for 2 channels */
    ui8 *pbAudioData;

#if defined _TMG_WINDOWS || defined _TMG_MAC || defined _TMG_LINUX ||          \
    defined _TMG_MACOSX || defined _TMG_QNX6
    void (*pFnErrorHandler)(ui32, char *, ui32,
                            char *); /* Pointer to error handler function */
#else                                /* _TMG_DOS32 */
    void(EXPORT_FN *pFnErrorHandler)(
        ui32, char *, ui32, char *); /* Pointer to error handler function */
#endif
...
struct tTMG_HistogramStatistics {
    ...
    /* Standard deviation is computationally intense, so we can select whether
     * to do that or not */
 ...
 * ----------
 */
ui32 EXPORT_FN TMG_Image_Create(int *phImage, int nSizeInBytes,
                                void *pVoid);     /* Thread/alignment safe */
...
ui32 EXPORT_FN TMG_Image_FileRead(
    int hImage); /* TIFF/BMP thread/alignment safe, JPEG threadsafe */
ui32 EXPORT_FN TMG_Image_FileWrite(
    int hImage,
    int nFormat); /* TIFF/BMP thread/alignment safe, JPEG threadsafe */

ui32 EXPORT_FN TMG_Image_FilenameSet(
    int hImage, int nParameter, char *pszFilename); /* Thread/alignment safe */
ui32 EXPORT_FN TMG_Image_FilenameGet(
    int hImage, int nParameter, char *pszFilename); /* Thread/alignment safe */

ui32 EXPORT_FN TMG_Image_FlagsSet(int hImage, ui32 dwFlags,
                                  int nSetting); /* Thread/alignment safe */
int EXPORT_FN TMG_Image_FlagsGet(int hImage,
                                 ui32 dwFlags); /* Thread/alignment safe */

ui32 EXPORT_FN TMG_Image_ParameterSet(int hImage, int nParameter,
                                      int nSetting); /* Thread/alignment safe */
ui32 EXPORT_FN TMG_Image_ParameterGet(
    int hImage, int nParameter, int *pnSetting); /* Thread/alignment safe */

ui32 EXPORT_FN TMG_Image_ParameterPtrGet(
    int hImage, int nParameter, void **ppVoid); /* Thread/alignment safe */
ui32 EXPORT_FN TMG_Image_ParameterPtrSet(
    int hImage, int nParameter, void *pVoid); /* Thread/alignment safe */
...
ui32 EXPORT_FN TMG_Display_Init(Tdisplay_handle hDisplay,
                                HWND hWnd); /* Thread/alignment safe */
ui32 EXPORT_FN TMG_Display_Image(Tdisplay_handle hDisplay, ui32 hImage,
                                 int nTargetPlane);
ui32 EXPORT_FN TMG_Display_ParameterGetPtr(Tdisplay_handle hDisplay,
                                           int nParameter, void **ppVoid);

ui32 EXPORT_FN TMG_Display_VideoOverlaySetSourceSize(Tdisplay_handle hDisplay,
                                                     int nWidth, int nHeight,
                                                     ui32 dwPixelFormat);
ui32 EXPORT_FN TMG_Display_VideoOverlaySetTargetSize(Tdisplay_handle hDisplay,
                                                     i16 gTargetRoi[]);

ui32 EXPORT_FN TMG_Draw_Text(int hImage, char *pszString, int nFont,
                             int nX_Across, int nY_Down, int nBlendMode,
                             int nColor);
 ...
 * ---------
 */
ui32 EXPORT_FN TMG_IP_ColorBalance(int hImageIn, int hImageOut, int nR, int nG,
                                   int nB);
ui32 EXPORT_FN TMG_IP_Crop_Image(int hImageIn, int hImageOut,
                                 ui32 naRoi[]); /* Thread/alignment safe */
ui32 EXPORT_FN TMG_IP_ImageOperation(int hImage1, int hImage2, int nOperation);
ui32 EXPORT_FN TMG_IP_ImageOperation_Matrix3x3(int hImage, float fMatrix[9],
                                               int nOffset[3]);
ui32 EXPORT_FN TMG_IP_PixelOperation(int hImage, int nOperation,
                                     ui32 dwOperation);

ui32 EXPORT_FN
TMG_IP_Image_Convert(int hImageIn, int hImageOut, int nPixelFormat,
                     ui32 dwFlags); /* Thread/alignment safe:
                                       convert_to_Y8,Y16,BGRX32,RGB24,BGR24 */
ui32 EXPORT_FN TMG_IP_Image_Copy(int hImageIn,
                                 int hImageOut); /* Thread/alignment safe */

ui32 EXPORT_FN TMG_IP_JPEG_Compress(int hImageIn,
                                    int hImageOut); /* Threadsafe */
ui32 EXPORT_FN TMG_IP_JPEG_Decompress(int hImageIn,
                                      int hImageOut); /* Threadsafe */

ui32 EXPORT_FN TMG_IP_Field_to_Frame(int hImageIn, int hImageOut, int nMethod);
ui32 EXPORT_FN TMG_IP_Frame_to_Field(int hImageIn, int hImageOut,
                                     int nFieldSelect);

Terr EXPORT_FN TMG_IP_Subsample(int hInImage, int hOutImage, int nFactor);
Terr EXPORT_FN TMG_IP_Subsample_by_Binning(
    int hInImage, int hOutImage,
    int nFactor); /* In tmg_scl.c, maybe move to tmg2_scl.c? */

/* tmg2_bayer.c
 * ------------
 */
ui32 EXPORT_FN TMG_IP_Bayer_Convert_to_BGRX32(int hImageIn, int hImageOut,
                                              int nDecodeScheme,
                                              float fColorMatrix[9],
                                              int nOffset[3],
                                              int nTopLeftPixel);
ui32 EXPORT_FN TMG_IP_Bayer_Convert_to_ThreePlanes(int hImageIn, int hImageOutR,
                                                   int hImageOutG,
                                                   int hImageOutB,
                                                   int nTopLeftPixel);
ui32 EXPORT_FN TMG_IP_Bayer_Map_to_RGB24(int hImageIn, int hImageBayer_RGB,
                                         int nTopLeftPixel);
Terr EXPORT_FN TMG_IP_Bayer_Convert_to_BGRX32_Sub2(int hImageIn, int hImageOut,
                                                   int nTopLeftPixel);

/* tmg2_histogram_stats.c
 * ----------------------
 */
ui32 EXPORT_FN TMG_IP_Histogram_Clear(
    struct tTMG_HistogramStatistics *psTMG_HistogramStatistics);
ui32 EXPORT_FN TMG_IP_Histogram_Statistics(
    int hImage, struct tTMG_HistogramStatistics *psTMG_HistogramStatistics);
...
ui32 EXPORT_FN TMG_Stream_FilenameGet(int hDVR, int nParameter,
                                      char *pszFilename);
ui32 EXPORT_FN TMG_Stream_FilenameSet(int hDVR, int nParameter,
                                      char *pszFilename);

ui32 EXPORT_FN TMG_Stream_ParameterGet(int hDVR, int nParameter,
                                       int *pnSetting);
ui32 EXPORT_FN TMG_Stream_ParameterSet(int hDVR, int nParameter, int nSetting);

ui32 EXPORT_FN TMG_Stream_ParameterPtrGet(int hDVR, int nParameter,
                                          void **ppVoid);
ui32 EXPORT_FN TMG_Stream_ParameterPtrSet(int hDVR, int nParameter,
                                          void *pVoid);

ui32 EXPORT_FN TMG_Stream_FileRead(int hDVR, int hImage, int nIndex,
                                   int nFormat);
...
ui32 EXPORT_FN TMG_Stream_InfoSet(ui32 hHandle,
                                  struct tTMG_DVR *psDVR_FromUser);
