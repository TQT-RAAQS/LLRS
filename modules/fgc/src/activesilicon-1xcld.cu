...
*   @param roi_xoffset The x offset of the top-left corner of the ROI from the top-left corner of the camera sensor.
*   @param roi_yoffset The y offset of the top-left corner of the ROI from the top-left corner of the camera sensor.
*/
Acquisition::ActiveSilicon1XCLD::ActiveSilicon1XCLD(uint32_t roi_width, uint32_t roi_height, 
    uint32_t roi_xoffset, uint32_t roi_yoffset, uint32_t vbin, uint32_t hbin):
    _roi_width(roi_width), _roi_height(roi_height), _roi_xoffset(roi_xoffset), _roi_yoffset(roi_yoffset),
    _vbin(vbin), _hbin(hbin)
{
    /* Create handle and load default settings base on the PCF file */
	std::string NADA = "";
    std::string pcf_config_path = std::string(NADA + PROJECT_BASE_DIR + "/config/fgc/activesilicon_1xcld.pcf");
    ...
    PHX_ParameterSet(this->_handle, PHX_CAM_ACTIVE_XOFFSET, &this->_roi_xoffset);
    PHX_ParameterSet(this->_handle, PHX_CAM_ACTIVE_YOFFSET, &this->_roi_yoffset);
    ...
    PHX_ParameterSet(this->_handle, (etParam) (PHX_DUMMY_PARAM | PHX_CACHE_FLUSH), NULL );
    init_image_buffer();
}

/**
*   @brief Initializes the image buffer for ActiveSilicon1XCLD acquisition class.
*   This function initializes the image buffer for ActiveSilicon1XCLD acquisition class. If CUDA is enabled,
*   it allocates memory on the GPU and creates a GPUDirect buffer, otherwise it allocates memory on the CPU.
*/
void Acquisition::ActiveSilicon1XCLD::init_image_buffer() 
{
    #ifdef ENABLE_CUDA
        cudaMalloc((void**)&gpu_buffers_src, MAX_IMAGE_READ_BUFFER_SIZE*sizeof(uint16_t));
        /* gpu_buffer_desc-> a class stores information for the buffer not the buffer 
            it self create a GPUDirectBuffer use to create and allocate the buffer  */
        gpuObjectDesc gpu_buffer_desc = {False, 1024, 1024, 1, 2, 0, MAX_IMAGE_READ_BUFFER_SIZE*sizeof(uint16_t)};
        /* Initialise this object with the GPU object provided (allocates system memory for DMA, semaphore, etc.) */
    ...
    #else
        this->_image_buffers[0].pvAddress = new uint16_t[MAX_IMAGE_READ_BUFFER_SIZE];
        this->_image_buffers[1].pvAddress = NULL;
    #endif
...
std::vector<uint16_t> Acquisition::ActiveSilicon1XCLD::acquire_stored_image( const char* filename ){ 
...
*   @return A vector of uint16_t integers containing the pixel values of the acquired image.
*      If an error occurs or the acquisition times out, an empty vector is returned.
*/
std::vector<uint16_t> Acquisition::ActiveSilicon1XCLD::acquire_single_image() 
{
    ...
    if (status != 0){
        ret_image = get_current_buffer();                                   // get the current buffer contents when check_and_wait finishes
        stop_stream_read();                                                 // stop the stream read
        INFO << "Image Acquisition Worked" << std::endl;
        
    } else {
        //TODO: Log this instead of cerr
        ERROR << "WARNING: timed out on image acquistion" << std::endl;     // check_and_wait did not finish in a suitable time
    }
...
/**
*   @brief Retrieves the current buffer of acquired image data and transfers it to the host.
...
std::vector<uint16_t> Acquisition::ActiveSilicon1XCLD::get_current_buffer() 
{
    ...
    std::vector<uint16_t> ret_image(this->_roi_width * this->_roi_height, 0);
        ...
        gpu_buffer.TransferChunk(); //triggers the rdma transfer that is set up by get_frame
        cudaMemcpy(&ret_image[0], gpu_buffers_src , this->_roi_width * this->_roi_height * sizeof(uint16_t), cudaMemcpyDeviceToHost);
        gpu_buffer.EndFrame();
    #else
        memcpy(&ret_image[0], this->_image_buffers[0].pvAddress, this->_roi_width * this->_roi_height * sizeof(uint16_t));
    #endif
...
/**
*   @brief Checks and waits for an event from the frame grabber, using a timeout.
*   an event variable will be set to the value of the event that occurred.
...
int32_t Acquisition::ActiveSilicon1XCLD::check_and_wait() 
{
    ...
    boost::asio::deadline_timer timer(io_service, boost::posix_time::milliseconds(ACQ_TIMEOUT));
    ...
    timer.async_wait([&](const boost::system::error_code& error) {      // asynchronous timer
        if (!error && (event == 0)) {                                   // if no event occured and no error then abort the stream read
            abort_stream_read();
        }
        io_service.stop();                      // Determine whether the io_service object has been stopped
    });
...
*   This function deallocates any memory or resources that were allocated during the lifetime
*/
Acquisition::ActiveSilicon1XCLD::~ActiveSilicon1XCLD() 
{
    ...
    std::cout << "this->_image_buffers[0].pvAddress: " << this->_image_buffers[0].pvAddress << std::endl;
...
{
    if (this->_handle) {
        if (PHX_Destroy(&this->_handle) != PHX_OK){      // PHX_Destroy destroys the handle 
            return CAMERA_ERR;                             // return LLRS_ERR if destroying the handle does not occur
        }
    }
    return CAMERA_OK;
}
...
//Repackaging of driver calls from the PHX library for the Active Silicon Card indicated by _handle
