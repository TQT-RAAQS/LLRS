 ...
 */
etBufferStat CUDAArrayParamsToDVPParams(CUarray_format cudaInFormat,
                                        int nInNumChannels,
 ...
 */
etBufferStat OGLTextureParamsToDVPParams(GLenum glInFormat, GLenum glInType,
                                         DVPBufferFormats *pdvpOutFormat,
                                         DVPBufferTypes *pdvpOutType) {
    if (!pdvpOutFormat || !pdvpOutType) {
        return BUFFER_STAT_INVALID_PARAMETER;
 ...
 */
etBufferStat SetupSyncObject(syncInfo *psSyncInfo,
                             const stGPUMemInfo &sGPUMemInfo) {
    etBufferStat eReturnStatus = BUFFER_STAT_OK;

    DVPSyncObjectDesc syncObjectDesc;
    if (!sGPUMemInfo.dwSemaphoreAllocSize ||
        !sGPUMemInfo.dwSemaphoreAddrAlignment || !psSyncInfo) {
        return BUFFER_STAT_INVALID_PARAMETER;
    }
    psSyncInfo->pdwSemOrg =
        (uint32_t *)malloc(sGPUMemInfo.dwSemaphoreAllocSize +
                           sGPUMemInfo.dwSemaphoreAddrAlignment - 1);
    ...
    eReturnStatus = (etBufferStat)dvpImportSyncObject(&syncObjectDesc,
                                                      &psSyncInfo->dvpSyncObj);
...
CBuffer::CBuffer(etGPUAPI eGPUAPI, void *pvDevice)
    : m_eGPUAPIUsed(eGPUAPI), m_pvD3DDevice(pvDevice), m_fTexture(true),
      m_pvSysMemAlloc(0), m_pvSysMemBuffer(0), m_hDvpSysMemHandle(0),
      m_ExtSync(), m_hDvpGpuObjectHandle(0), m_GpuSync(), m_dwWidth(0),
      m_dwHeight(0), m_dwStride(0), m_dwChunks(0), m_nSize(0) {
#ifdef _WIN32
...
etBufferStat CBuffer::Init(gpuObjectDesc &desc, void *pvGPUObject,
                           int nBitDepth /* = 8 */
) {
    DVPSysmemBufferDesc
        sysMemBuffersDesc; /* struct with info about buffer (stride etc.) */
    ...
    if (!desc.fUseTexture) {
        ...
        case GPUAPI_CUDA:
            RETURN_ON_ERROR(CUDAArrayParamsToDVPParams(
                (CUarray_format)desc.dwFormat, desc.dwNumChannels,
                &sysMemBuffersDesc.format, &sysMemBuffersDesc.type));
            break;
        ...
        default:
            ...
            RETURN_ON_ERROR(OGLTextureParamsToDVPParams(
                (GLenum)desc.dwFormat, (GLenum)desc.dwType,
                &sysMemBuffersDesc.format, &sysMemBuffersDesc.type));
            break;
        }
        ...
        int nNumChannels =
            DVPBufferFormatToNumChannels(sysMemBuffersDesc.format);
        ...
        uint32_t dwBufferStride =
            desc.dwWidth * nNumChannels * nNumBytesPerChannel;
    ...
    case GPUAPI_DX11:
        if (desc.fUseTexture) {
            m_hGpuTextureHandle.gpuTextureHandleD3D11 =
                *((ID3D11Texture2D **)pvGPUObject);
        } else {
            m_hGpuBufferHandle.gpuBufferHandleD3D11 =
                *((ID3D11Buffer **)pvGPUObject);
        }
        break;

    case GPUAPI_DX9:
        if (desc.fUseTexture) {
            m_hGpuTextureHandle.gpuTextureHandleD3D9 =
                *((IDirect3DTexture9 **)pvGPUObject);
        } else {
            m_hGpuBufferHandle.gpuBufferHandleD3D9 =
                *((IDirect3DVertexBuffer9 **)pvGPUObject);
        }
        break;
#endif
    case GPUAPI_CUDA:
        if (desc.fUseTexture) {
            m_hGpuTextureHandle.gpuTextureHandleCUDA =
                *((CUarray *)pvGPUObject);
        } else {
            m_hGpuBufferHandle.gpuBufferHandleCUDA =
                *((CUdeviceptr *)pvGPUObject);
        }
        break;
...
#ifdef _WIN32
    m_pvSysMemBuffer = VirtualAlloc(NULL, sysMemBuffersDesc.size,
...
#ifdef _WIN32
    case GPUAPI_DX11:
        RETURN_ON_ERROR(dvpBindToD3D11Device(m_hDvpSysMemHandle,
                                             (ID3D11Device *)m_pvD3DDevice));
        break;

    case GPUAPI_DX9:
        RETURN_ON_ERROR(dvpBindToD3D9Device(m_hDvpSysMemHandle,
                                            (IDirect3DDevice9 *)m_pvD3DDevice));
        break;
#endif
    case GPUAPI_CUDA:
    ...
    case GPUAPI_DX11:
        if (desc.fUseTexture) {
            RETURN_ON_ERROR(dvpCreateGPUD3D11Resource(
                (ID3D11Resource *)m_hGpuTextureHandle.gpuTextureHandleD3D11,
                &m_hDvpGpuObjectHandle));
        } else {
            RETURN_ON_ERROR(dvpCreateGPUD3D11Resource(
                (ID3D11Resource *)m_hGpuBufferHandle.gpuBufferHandleD3D11,
                &m_hDvpGpuObjectHandle));
        }
        break;
    ...
    case GPUAPI_DX9:
        if (desc.fUseTexture) {
            RETURN_ON_ERROR(dvpCreateGPUD3D9Resource(
                (IDirect3DResource9 *)m_hGpuTextureHandle.gpuTextureHandleD3D9,
                &m_hDvpGpuObjectHandle));
        } else {
            RETURN_ON_ERROR(dvpCreateGPUD3D9Resource(
                (IDirect3DResource9 *)m_hGpuBufferHandle.gpuBufferHandleD3D9,
                &m_hDvpGpuObjectHandle));
        }
        break;
#endif
    case GPUAPI_CUDA:
        if (desc.fUseTexture) {
            RETURN_ON_ERROR(
                dvpCreateGPUCUDAArray(m_hGpuTextureHandle.gpuTextureHandleCUDA,
                                      &m_hDvpGpuObjectHandle));
        } else {
            RETURN_ON_ERROR(dvpCreateGPUCUDADevicePtr(
                m_hGpuBufferHandle.gpuBufferHandleCUDA,
                &m_hDvpGpuObjectHandle));
        }
        break;
    ...
    default:
        if (desc.fUseTexture) {
            RETURN_ON_ERROR(
                dvpCreateGPUTextureGL(m_hGpuTextureHandle.gpuTextureHandleGL,
                                      &m_hDvpGpuObjectHandle));
        } else {
            RETURN_ON_ERROR(dvpCreateGPUBufferGL(
                m_hGpuBufferHandle.gpuBufferHandleGL, &m_hDvpGpuObjectHandle));
        }
        break;
    }
...
etBufferStat CBuffer::GetFrame() {
    ...
    /* We want to acquire the previous transfer's release value on the first
     * call to make sure that we don't corrupt the previous data. This is likely
     * to happen if the input is faster than the app processing. Make sure we
     * are done with the pending GPU DMAs.
     */

    /* Wait for GPU API to be done rendering buffer (signalled by
     * MapBufferEndAPI() before we attempt to DMA data into it */
    START_TRACE
    ...
    if (m_fTexture) {
        ...
        while (dwCopiedLines < m_dwHeight) {
            ...
            uint32_t dwLinesToCopy =
                (m_dwHeight - dwCopiedLines > dwNumLinesPerCopy
                     ? dwNumLinesPerCopy
                     : m_dwHeight - dwCopiedLines);

            m_ExtSync.dwAcquireValue++;
            m_GpuSync.dwReleaseValue++;

            START_TRACE
            RETURN_ON_ERROR(dvpMemcpyLined(
                m_hDvpSysMemHandle, m_ExtSync.dvpSyncObj,
                m_ExtSync.dwAcquireValue, DVP_TIMEOUT_IGNORED,
                m_hDvpGpuObjectHandle, m_GpuSync.dvpSyncObj,
                m_GpuSync.dwReleaseValue, dwCopiedLines, dwLinesToCopy));
        ...
        while (dwCopiedLines < m_dwHeight) {
            ...
            START_TRACE
            RETURN_ON_ERROR(
                dvpMemcpy2D(m_hDvpSysMemHandle, m_ExtSync.dvpSyncObj,
                            m_ExtSync.dwAcquireValue, DVP_TIMEOUT_IGNORED,
                            m_hDvpGpuObjectHandle, m_GpuSync.dvpSyncObj,
                            m_GpuSync.dwReleaseValue, 0, dwCopiedLines,
                            dwLinesToCopy, m_dwWidth));
        ...
        while (dwCopiedSize < (uint32_t)m_nSize) {
            uint32_t dwCopiedChunkSize =
                ((uint32_t)m_nSize - dwCopiedSize > dwChunkSize
                     ? dwChunkSize
                     : (uint32_t)m_nSize - dwCopiedSize);
            m_ExtSync.dwAcquireValue++;
            m_GpuSync.dwReleaseValue++;

            START_TRACE
            RETURN_ON_ERROR(
                dvpMemcpy(m_hDvpSysMemHandle, m_ExtSync.dvpSyncObj,
                          m_ExtSync.dwAcquireValue, DVP_TIMEOUT_IGNORED,
                          m_hDvpGpuObjectHandle, m_GpuSync.dvpSyncObj,
                          m_GpuSync.dwReleaseValue, dwCopiedSize, dwCopiedSize,
                          dwCopiedChunkSize));
            END_TRACE(dvpMemcpy);
    ...
    START_TRACE
    /* Signal to GPU API that the GPU Object can now be used (waited for via
     * MapBufferWaitAPI()) */
...
etBufferStat CBuffer::WaitGPUDoneWithObject() {
    SETUP_TRACE
    /* Wait for DMA to GPU to have completed and GPU object to be usable
     * (signaled via dvpMapBufferEndDVP()) */
    if (m_eGPUAPIUsed == GPUAPI_CUDA) {
        CUstream stream = 0;
        START_TRACE
        RETURN_ON_ERROR(
            dvpMapBufferWaitCUDAStream(m_hDvpGpuObjectHandle, stream));
...
etBufferStat CBuffer::EndFrame() {
    ...
    if (m_eGPUAPIUsed == GPUAPI_CUDA) {
        ...
        START_TRACE
        RETURN_ON_ERROR(
            dvpMapBufferEndCUDAStream(m_hDvpGpuObjectHandle, stream));
...
etBufferStat CBuffer::GetGPUMemInfo(etGPUAPI eGPUAPI, void *pD3DDevice,
    ...
    if (!psGPUMemInfo) {
        ...
        case GPUAPI_DX11: {
            ...
            RETURN_ON_ERROR(dvpGetRequiredConstantsD3D11Device(
                &sGPUMemInfo.dwBufferAddrAlignment,
                &sGPUMemInfo.dwBufferGPUStrideAlignment,
                &sGPUMemInfo.dwSemaphoreAddrAlignment,
                &sGPUMemInfo.dwSemaphoreAllocSize,
                &sGPUMemInfo.dwSemaphorePayloadOffset,
                &sGPUMemInfo.dwSemaphorePayloadSize, pD3Ddev));
        } break;

        case GPUAPI_DX9: {
            IDirect3DDevice9 *pD3Ddev = (IDirect3DDevice9 *)pD3DDevice;
            RETURN_ON_ERROR(dvpInitD3D9Device(pD3Ddev, 0));
            RETURN_ON_ERROR(dvpGetRequiredConstantsD3D9Device(
                &sGPUMemInfo.dwBufferAddrAlignment,
                &sGPUMemInfo.dwBufferGPUStrideAlignment,
                &sGPUMemInfo.dwSemaphoreAddrAlignment,
                &sGPUMemInfo.dwSemaphoreAllocSize,
        ...
        case GPUAPI_CUDA: {
            RETURN_ON_ERROR(
                dvpInitCUDAContext(DVP_DEVICE_FLAGS_SHARE_APP_CONTEXT));
            RETURN_ON_ERROR(dvpGetRequiredConstantsCUDACtx(
                &sGPUMemInfo.dwBufferAddrAlignment,
                &sGPUMemInfo.dwBufferGPUStrideAlignment,
                &sGPUMemInfo.dwSemaphoreAddrAlignment,
                &sGPUMemInfo.dwSemaphoreAllocSize,
                &sGPUMemInfo.dwSemaphorePayloadOffset,
                &sGPUMemInfo.dwSemaphorePayloadSize));
        } break;

        case GPUAPI_GL: {
            RETURN_ON_ERROR(dvpInitGLContext(0));
            RETURN_ON_ERROR(dvpGetRequiredConstantsGLCtx(
                &sGPUMemInfo.dwBufferAddrAlignment,
                &sGPUMemInfo.dwBufferGPUStrideAlignment,
                &sGPUMemInfo.dwSemaphoreAddrAlignment,
                &sGPUMemInfo.dwSemaphoreAllocSize,
...
etBufferStat CBuffer::DeInit() {
    ...
    if (m_eGPUAPIUsed == GPUAPI_DX11) {
        RETURN_ON_ERROR(dvpUnbindFromD3D11Device(
            m_hDvpSysMemHandle, (ID3D11Device *)m_pvD3DDevice));
    } else if (m_eGPUAPIUsed == GPUAPI_DX9) {
        RETURN_ON_ERROR(dvpUnbindFromD3D9Device(
            m_hDvpSysMemHandle, (IDirect3DDevice9 *)m_pvD3DDevice));
    }
#endif
    if (m_eGPUAPIUsed == GPUAPI_CUDA) {
...
void *CBuffer::GetSystemMemory() { return m_pvSysMemBuffer; }
