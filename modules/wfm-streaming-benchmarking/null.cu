...
int main(int argc, char * argv[]) { // <Number of moves>, <numbers of waveforms per segment>, <num of reps>
    ...
    /*{
        std::string problem_config = std::string("") + "# AWG Global Settings \n \
        ...
        waveforms_per_segment:    " + std::to_string(i) + "      # waveforms per segment : int  \n \
        ...
        std::ofstream config_file {std::string("") + PROJECT_BASE_DIR + "/configs/awg/awg.yml"};
    ...
    wf_table = Setup::create_wf_table(
        ...
        wfm_mask, vpp, "21_traps.csv",
        "21_traps.csv", true);
    awg_sequence.setup(true, 0, false, Nt_x, Nt_y);    
    awg_sequence.start_stream();
   

    std::vector<int> nums = {64,128,256,512,1024};
    for (auto t:nums) {
        std::cout << "Number of moves: " << t << std::endl;
        for (int j = 0; j < 10; j++) { // warm up the GPU
            int counter = 0;
...
std::vector<Reconfig::Move> generate_moves(int number_of_moves) {
    ...
    for(int i = 0; i < number_of_moves; i++) { // this function can be changed to generate random moves, even though it will not affect timing or the result of the benchmark
