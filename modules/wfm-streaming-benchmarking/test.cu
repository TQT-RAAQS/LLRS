/*
This test is useful for streaming waveforms on both output channels of the AWG. Make sure to change
the configs of the awg to stream on both channels with a save Vpp and amplitudes.
*/
...
int main() {
    ...
    std::vector<short> wf1 = translate(Synthesis::Idle(wf_duration, std::make_tuple(alpha1, nu1, phi1))
        .discretize(sample_rate));
    std::vector<short> wf2 = translate(Synthesis::Idle(wf_duration, std::make_tuple(alpha2, nu2, phi2))
        .discretize(sample_rate));
    ...
    awg.init_and_load_range(
        *buffer, static_cast<int>(wf_duration * sample_rate),
        0, 1);

    awg.seqmem_update(0, 0, 1,
                      0, SPCSEQ_ENDLOOPONTRIG);
