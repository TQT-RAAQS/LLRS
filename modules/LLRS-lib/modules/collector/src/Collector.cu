...
void Util::Collector::get_external_time(const std::string& module, int trial, int rep, int cycle, float time){
    ...
    _timers[module][trial][rep][cycle] = std::make_pair(time_now, time_now_plus_alg_elapse);
}

void Util::Collector::start_timer(const std::string& module, int trial, int rep, int cycle) {
    if (_timers.find(module) == _timers.end()) {
        _timers[module] = TRIAL_INFO();
    }
    while (_timers[module].size() <= trial) {
...
void Util::Collector::end_timer(const std::string& module, int trial, int rep, int cycle) {
    auto finish = std::chrono::high_resolution_clock::now();
    _timers[module][trial][rep][cycle].second = finish;
}

long long Util::Collector::elapsed_time(const std::string& module, int trial, int rep, int cycle) {
    if (_timers.find(module) == _timers.end()){
        return 0;
    }
        ...
        for (int i = 0; i < trial_info.size(); ++i) {
             ...
             for (int j = 0; j < rep_info.size(); ++j){
                ...
                for (int k = 0; k < cycle_info.size(); k++){
                    ...
                    root[trial_name][repetition_name][cycle_name][module] = elapsed_time(module, i, j, k);
                }
             }
        }
    }
