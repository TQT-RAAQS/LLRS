...
void populate_hungarian_indices_to_original_indices(int* source, int* target, int N, int hungarian_index_to_source_index[], int hungarian_index_to_target_index[]) {
...
void batched_output_generator_cpu(int matching_src[], int matching_dst[], int K_prime, int* src_batched, int* dst_batched, int* batchPtr_batched, int* sol_length_batched, int* num_batches_batched) {
	startTime(&timer);
    // For each element of the matching, compute the distance from source to target
...
int get_cost(int compressed_src[], int starting_index, int target_start, int K_prime) {
    int total_cost = 0;
    for (int i = 0; i < K_prime; i++) {
        total_cost += abs(compressed_src[starting_index + i] - (target_start + i));
    }
    return total_cost;
}
...
// Corner cases:
// 1- Everything is before start of target: handled by first_atom_after_target_start = K;
// 2- Everything is after start of target: startIndex will be equal to endIndex
// Note that startIndex >= endIndex
void bruteforce_generate_matching(int* source, int N, int K, int K_prime, int K_1, int K_2, int matching_src[], int matching_dst[]) {
    // Compress source array
    int compressed_src[K];
    int compressed_idx = 0;
    // This handles the case where there are no atoms after target start (when this is set to K, startingIndex = endingIndex = K - K_prime)
    ...
    // Ensure that there are at least K_prime atoms to the right, so if first_atom_after_target_start
    // is too "to the right", the first split might not be 0 atoms from before target, it might be more
    int startingIndex = min(K - K_prime, first_atom_after_target_start);
    // This part is clear: starting from leftmost atom after target start, select up to decifit/atoms_before_target
    // atoms from before target start, whichever is smaller
    int endingIndex = first_atom_after_target_start - min(atoms_before_target, deficit);
...
void bruteforce_generate_moves_batched_cpu(int* source, int N, int K, int K_prime, int K_1, int K_2, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    // Matching that corresponds to the best split
    int matching_src[K_prime];
    int matching_dst[K_prime];

    bruteforce_generate_matching(source, N, K, K_prime, K_1, K_2, matching_src, matching_dst);

    batched_output_generator_cpu(matching_src, matching_dst, K_prime, src_batched, dst_batched, batchPtr_batched, sol_length_batched, num_batches_batched);
}

// This function takes in results from a 1D solver call, converts them, executes them, then adds them
// to the overall output of the 2D solver
void convert_and_execute_1d_solver_output(int* source, int W, int d_c, int* src_batched_1d, int* dst_batched_1d, int* batchPtr_batched_1d, int num_batches_batched_1d, int sol_length_batched_1d, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
	startTime(&timer);
    ...
    for (int i = 0; i < num_batches_batched_1d; i++) {
        // Save batch ptr
        batchPtr_batched[*num_batches_batched + i] = *sol_length_batched + local_displacements_executed;
        for (int j = batchPtr_batched_1d[i]; j < (i == num_batches_batched_1d - 1 ? sol_length_batched_1d : batchPtr_batched_1d[i + 1]); j++) {
            ...
            src_batched[*sol_length_batched + local_displacements_executed] = src*W + d_c;
            dst_batched[*sol_length_batched + local_displacements_executed] = dst*W + d_c;
...
// This function is almost the same as the one above, except it does not do any conversion
// This function is used on the output of shuffling
void execute_shuffling_output(int* source, int* src_batched_1d, int* dst_batched_1d, int* batchPtr_batched_1d, int num_batches_batched_1d, int sol_length_batched_1d, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    startTime(&timer);
	int local_displacements_executed = 0;
    ...
    for (int i = 0; i < num_batches_batched_1d; i++) {
        ...
        for (int j = batchPtr_batched_1d[i]; j < (i == num_batches_batched_1d - 1 ? sol_length_batched_1d : batchPtr_batched_1d[i + 1]); j++) {
            ...
            source[dst]++;
            // Add the move to the output
            src_batched[*sol_length_batched + local_displacements_executed] = src;
            dst_batched[*sol_length_batched + local_displacements_executed] = dst;
            // Increment the number of local displacements
            local_displacements_executed++;
        }
    }
...
// matching generator
void lin_exact_generate_matching(int* source, int* target, int N, int K, int K_prime, int matching_src[], int matching_dst[]) {
    int X = N + 1;
    // Including 0; H[0] = 0 and H[N + 1] = e
    int H[N + 2];

    // Offset the source just so that the sources and targets are in (0, X) (set X to N + 1)
    ...
    // We can get the corresponding k using H, the below arrays store the indices within the k
    ...
    // pi contains the maximum profit needed for heights between 1 and e (inclusive), so we just loop 
    // over the values now!
    for (int i = 0; i < K; i++) {
        int y = S_set[i];
...
void lin_exact_generate_moves_batched_cpu(int* source, int* target, int N, int K, int K_prime, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
...
int get_top_reservoir_displacement(int* source, int* atoms_selected_from_top, int num_traps_selected_from_top, int R_h, int H, int W, int r_c) {
    int displacement = 0;
    int cur_row = R_h;
    int cur_trap = 0;
...
int get_bottom_reservoir_displacement(int* source, int* atoms_selected_from_bottom, int num_traps_selected_from_bottom, int R_h, int H, int W, int r_c) {
    int displacement = 0;
    int cur_row = H - R_h - 1;
...
int coord_to_idx(int r, int c, int W) {
    return r * W + c;
}
...
void generate_and_execute_center_batches(int* source, int* matching_src, int* matching_dst, int W, int matching_length, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    startTime(&timer);
	// Get the required number of batches
    int local_num_batches = 0;
    for (int i = 0; i < matching_length; i++) {
        local_num_batches = max(local_num_batches, abs(matching_src[i] - matching_dst[i]) / W);
    }
    ...
    for (int batch_index = 0; batch_index < local_num_batches; batch_index++) {
        batchPtr_batched[*num_batches_batched + batch_index] = *sol_length_batched + local_displacements;
        for (int i = 0; i < matching_length; i++) {
            if (matching_src[i] < matching_dst[i]) {
                src_batched[*sol_length_batched + local_displacements] = matching_src[i];
                dst_batched[*sol_length_batched + local_displacements] = matching_src[i] + W;
...
void generate_centered_matching(int* source, int H, int W, int c, int starting_row, int* matching_src, int* matching_dst) {
...
void center_sort(int* source, int c, int num_atoms_in_c, int starting_row, int H, int W, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    // This is where we will be storing the matching
    int matching_src[num_atoms_in_c];
    int matching_dst[num_atoms_in_c];

    generate_centered_matching(source, H, W, c, starting_row, matching_src, matching_dst);

    // Generate batches, add them to output and execute them
    generate_and_execute_center_batches(source, matching_src, matching_dst, W, num_atoms_in_c, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);
}

void partial_sort(int* source, int c, int* surplus, int T_h, int H, int W, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    ...
    center_sort(source, c, num_atoms_in_c, (H - num_atoms_in_c) / 2, H, W, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);
}

void sort_net_zero_column(int* source, int c, int T_h, int R_h, int H, int W, int* dynamic_columns, int* column_solved, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    // If we are here, we know exactly how many atoms there are in the column (T_h)
    // and we know where center sort should start from (R_h)
    center_sort(source, c, T_h, R_h, H, W, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);
...
void sort_net_zero_columns(int* source, int H, int W, int T_h, int R_h, int* surplus, int* dynamic_columns, int* column_solved, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    for (int c = 0; c < W; c++) {
        if (surplus[c] == 0) {
            sort_net_zero_column(source, c, T_h, R_h, H, W, dynamic_columns, column_solved, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);
        }
    }
}
...
void find_pair_to_shuffle(int* surplus, int* column_solved, int* dynamic_columns, int* d, int* r, int W) {
    int atoms_to_shuffle = -INF;
    for (int i = 0; i < W - 1; i++) {
        // Check if the current column has not been solved and that it has an unsolved column
        // to its right, then check whether they make up a valid donor-receiver pair
        if (!column_solved[i] && dynamic_columns[i+1] != -1 && (surplus[i] * surplus[dynamic_columns[i+1]] < 0)) {
            atoms_to_shuffle = max(atoms_to_shuffle, min(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]])));
        }
    }
    // Now that we know the maximum number of atoms that we can shuffle, we pick the pair that minimizes
    // the difference of the absolute values, i.e. that gets both columns as close to completion as possible
    int selected_pair_index = -1;
    int abs_val_diff = INF;
    for (int i = 0; i < W - 1; i++) {
        if (!column_solved[i] && dynamic_columns[i+1] != -1 && (surplus[i] * surplus[dynamic_columns[i+1]] < 0)
        &&
        (min(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]])) == atoms_to_shuffle)) {
            // We have a valid candidate
            int max_abs_val = max(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]]));
            int min_abs_val = min(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]]));
...
void find_pairs_to_shuffle(int* surplus, int* column_solved, int* dynamic_columns, int* donors, int* receivers, int* num_pairs_to_shuffle, int max_num_pairs_to_shuffle, int W) {
    *num_pairs_to_shuffle = 0;
    ...
    // Here, we will save the atoms all pairs can shuffle (then we will use this information to sort the pairs)
    int atoms_to_shuffle[W];
    // We will also save the absolute value differences in case a tiebreaker is needed
    int abs_val_diff[W];
    ...
    for (int i = 0; i < W - 1; i++) {
        ...
        // to its right, then check whether they make up a valid donor-receiver pair
        if (!column_solved[i] && dynamic_columns[i+1] != -1 && (surplus[i] * surplus[dynamic_columns[i+1]] < 0)) {
            // Found a candidate pair, so compute the number of atoms it can shuffle and save this value in atoms_to_shuffle
            compressed_to_original[compressed_to_original_idx] = i;
            atoms_to_shuffle[compressed_to_original_idx] = min(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]]));
            
            // Compute absolute value difference in case a tiebreaker is needed
            // Smaller absolute value differences are preferred because they get pairs of columns to completion quicker
            int max_abs_val = max(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]]));
            int min_abs_val = min(abs(surplus[i]), abs(surplus[dynamic_columns[i+1]]));
            abs_val_diff[compressed_to_original_idx] = max_abs_val - min_abs_val;
    ...
    for (int i = 0; i < compressed_to_original_idx - 1; i++) {
        for (int j = 0; j < compressed_to_original_idx - i - 1; j++) {
            ...
            ||
            (atoms_to_shuffle[j] == atoms_to_shuffle[j + 1] && abs_val_diff[j] > abs_val_diff[j + 1])) {
    ...
    // Now that the arrays are sorted from best to worst, move the top max_num_pairs_to_shuffle pairs to the donors
    // and the receivers arrays

    // We need to keep track of the columns that have already been matched/paired so as not to pick them twice
    int is_locked[W];
    for (int i = 0; i < W; i++) {
        is_locked[i] = 0;
    }

    // We may shuffle less than possible_num_pairs_to_shuffle in case possible_num_pairs_to_shuffle is
    // equal to compressed_to_original_idx and the pairs are not mutually exclusive
    int possible_num_pairs_to_shuffle = min(compressed_to_original_idx, max_num_pairs_to_shuffle);
    int cur_compressed_idx = 0;
    int cur_output_index = 0;

    while (possible_num_pairs_to_shuffle && (cur_compressed_idx < compressed_to_original_idx)) {
...
int verify_redrec_solution(int* source, int num_source_atoms, int H, int W, int R_h) {
...
void sort_donor_and_shuffle(int* source, int H, int W, int R_h, int T_h, int atoms_to_shuffle, int d_c, int r_c, int* src_batched, int* dst_batched, int* batchPtr_batched, int* num_batches_batched, int* sol_length_batched) {
    ...
    for (atoms_from_top = 0; atoms_from_top <= atoms_to_shuffle; atoms_from_top++) {
        atoms_from_bottom = atoms_to_shuffle - atoms_from_top;
        // Check whether we have enough atoms on top & bottom for current split
        if (atoms_from_bottom <= num_vacancies_bottom && atoms_from_top <= num_vacancies_top) {
            int cur_target[H];
            ...
            for (int i = num_vacancies_top - atoms_from_top; i < num_vacancies_top; i++) {
                cur_target[vacant_traps_in_top[i]] = 1;
                atoms_selected_from_top[atoms_selected_from_top_idx++] = vacant_traps_in_top[i];
            }

            for (int i = 0; i < atoms_from_bottom; i++) {
                cur_target[vacant_traps_in_bottom[i]] = 1;
                atoms_selected_from_bottom[atoms_selected_from_bottom_idx++] = vacant_traps_in_bottom[i];
            }

            // Use the 1D Solver
            lin_exact_generate_moves_batched_cpu(source_arr, cur_target, H, K, K_prime, src_batched_1d, dst_batched_1d, batchPtr_batched_1d, &num_batches_batched_1d, &sol_length_batched_1d);

            // Displacement from the 1D Solver is equal to the length of the solution
            int displacement = sol_length_batched_1d;

            int top_reservoir_receiver_displacement = get_top_reservoir_displacement(source, atoms_selected_from_top, atoms_selected_from_top_idx, R_h, H, W, r_c);
            int bottom_reservoir_receiver_displacement = get_bottom_reservoir_displacement(source, atoms_selected_from_bottom, atoms_selected_from_bottom_idx, R_h, H, W, r_c);

            // Compute total displacement
            int total_displacement = displacement + top_reservoir_receiver_displacement + bottom_reservoir_receiver_displacement;
    ...
    convert_and_execute_1d_solver_output(source, W, d_c, src_batched_1d, dst_batched_1d, batchPtr_batched_1d, num_batches_batched_1d, sol_length_batched_1d, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);

    // To the above, we still have to add the reshuffling batches
    // The number of reshuffling batches is equal to the distance between the two columns
    int distance_between_columns = abs(d_c - r_c);
    // Direction depends on the position of the donor with respect to the receiver
    int direction = (r_c > d_c) ? 1 : -1;
    // The length of the solution here is known and is equal to the number of atoms/
    // to be shuffled multiplied by the distance between the columns
    ...
    // The number of batches is known too and is equal to the distance between the columns
    int reshuffling_batchPtr_batched[distance_between_columns];
    // Despite the fact that those are known, we will use them as indices, so they start at 0
    int reshuffling_num_batches_batched = 0;
    int reshuffling_sol_length_batched = 0;

    for (int i = 0; i < distance_between_columns; i++) {
        reshuffling_batchPtr_batched[reshuffling_num_batches_batched++] = reshuffling_sol_length_batched;
        ...
        for (int j = 0; j < atoms_from_top; j++) {
            ...
            reshuffling_src_batched[reshuffling_sol_length_batched] = coord_to_idx(atom_row, from_col, W);
            reshuffling_dst_batched[reshuffling_sol_length_batched] = coord_to_idx(atom_row, to_col, W);
    ...
    execute_shuffling_output(source, reshuffling_src_batched, reshuffling_dst_batched, reshuffling_batchPtr_batched, reshuffling_num_batches_batched, reshuffling_sol_length_batched, src_batched, dst_batched, batchPtr_batched, num_batches_batched, sol_length_batched);
}


float redrec_v2(int* initial, int initial_atom_count, int Nt_x, int Nt_y, int R_h, int* src, int* dst, int* batch_indices, int* num_batches, int* sol_length) {
    ...
    *num_batches = 0;
    ...
    // For all columns where the deficit is bigger than 0 and is bigger than the vacant spots in the reservoir, sort (everything goes to the middle)
    for (int c = 0; c < Nt_x; c++) {
        if (surplus[c] < 0 && abs(surplus[c]) > get_vacant_reservoir_traps(initial, Nt_y, Nt_x, R_h, c)) {
            partial_sort(initial, c, surplus, T_h, Nt_y, Nt_x, src, dst, batch_indices, num_batches, sol_length);
        }
    }

    // Sort columns that have a net surplus of zero
    sort_net_zero_columns(initial, Nt_y, Nt_x, T_h, R_h, surplus, dynamic_columns, column_solved, src, dst, batch_indices, num_batches, sol_length);
    ...
    while (deficit_exists(surplus, Nt_x)) {
        ...
        // The criteria we are using to determine this pair can be changed later on
        find_pair_to_shuffle(surplus, column_solved, dynamic_columns, &d, &r, Nt_x);

        int donor_surplus = surplus[d];
        int receiver_deficit = abs(surplus[r]);

        sort_donor_and_shuffle(initial, Nt_y, Nt_x, R_h, T_h, min(donor_surplus, receiver_deficit), d, r, src, dst, batch_indices, num_batches, sol_length);

        if (donor_surplus == receiver_deficit) {
            // Donor is already taken care of; we need to sort the receiver and mark it as solved
            ...
            column_solved[d] = 1;
            // Update dynamic columns (which should be done every time a column is marked as solved)
            update_dynamic_columns(Nt_x, column_solved, dynamic_columns);

            // Finally, solve the receiver
            sort_net_zero_column(initial, r, T_h, R_h, Nt_y, Nt_x, dynamic_columns, column_solved, src, dst, batch_indices, num_batches, sol_length);
        }
            ...
            if (abs(surplus[r]) > get_vacant_reservoir_traps(initial, Nt_y, Nt_x, R_h, r)) {
                partial_sort(initial, r, surplus, T_h, Nt_y, Nt_x, src, dst, batch_indices, num_batches, sol_length);
            }
        }
    }
    ...
    // Now that we are here, we know that the columns left are either already solved or require solving, use 1D solver for those
    for (int c = 0; c < Nt_x; c++) {
        if (!column_solved[c]) {
            ...
            bruteforce_generate_moves_batched_cpu(source_arr, Nt_y, K, K_prime, K_1, K_2, src_batched_1d, dst_batched_1d, batchPtr_batched_1d, &num_batches_batched_1d, &sol_length_batched_1d);
        
            convert_and_execute_1d_solver_output(initial, Nt_x, c, src_batched_1d, dst_batched_1d, batchPtr_batched_1d, num_batches_batched_1d, sol_length_batched_1d, src, dst, batch_indices, num_batches, sol_length);
        }
    }
