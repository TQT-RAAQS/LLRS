 ...
 * @return false 
 */
bool Util::target_met(std::vector<int32_t> current, std::vector<int32_t> target){
...
std::vector<int> Util::parse_pretty_vector(const std::string& str) {
    ...
    while (std::getline(ss, element_str, ',')) {
        int element;
        element_str.erase(std::remove_if(element_str.begin(), element_str.end(), ::isspace), element_str.end());
