...
enum State { IMPLANTED = 0, EXTRACTED = 1 };

/// defines time per move in seconds
#define SECONDS_PER_MOVE 10e-6

class Atom {
    // Atom class used to store properties and observables during
  ...
  public:
     ...
     * @param alpha_loss probability for atom to be lost when transferred
     * (default: 0.985)
     * @param nu_loss probability for atom to be lost when displaced (default:
     * 0.985)
     * @param lifetime lifetime of the atom in seconds (default: 60.0)
     */
    Atom(int n_alpha, int n_nu, State state, double alpha_loss, double nu_loss,
         double lifetime);

    /**
     * @brief Function to handle transfer of atom from the static to dynamic
     * array. Updates atom's number of transfers and switches its state
