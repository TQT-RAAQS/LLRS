...
Atom::Atom(int n_alpha, int n_nu, State state, double alpha_loss,
           double nu_loss, double lifetime) {
...
/**
 * @brief Function to handle transfer of atom from static to dynamic array.
 * Updates atom's number of transfers and switches it's state
...
/**
 * @brief Updates specific atom's number of displacements within the dynamic
 * array
...
int Atom::getAlpha() { return this->n_alpha; }
...
int Atom::getNu() { return this->n_nu; }
...
double Atom::getLifetime() { return this->lifetime; }
...
double Atom::getAlphaLoss() { return this->alpha_loss; }
...
double Atom::getNuLoss() { return this->nu_loss; }
...
State Atom::getState() { return this->state; }
...
void Atom::setState(State new_state) { this->state = new_state; }
...
double Atom::getLoss(int total_moves) {
    return std::pow(this->getAlphaLoss(), this->getAlpha()) *
           ...
           std::exp(((-1) * total_moves * SECONDS_PER_MOVE) /
                    this->getLifetime());
}
