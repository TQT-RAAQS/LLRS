...
void create_rectangular_target(std::vector<int32_t> &target_config,
                               int num_target, int Nt_x, int Nt_y) {
...
int main(int argc, char *argv[]) {
    ...
    create_rectangular_target(target_config, num_target, Nt_x, Nt_y);
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        ...
        // Shuffle the vector
        std::random_shuffle(trial_config.begin(), trial_config.end(),
                            [&](int n) { return g() % n; });
        // Start repetition loop
        for (int rep = 0; rep < num_reps; ++rep) {
            solver.start_solver(algo, trial_config, target_config);
            if (true) {
                auto moves = solver.gen_moves_list(algo, false);
                std::cout << moves.size() << " ";
            }
            data.push_back(
                batching ? Util::Collector::get_instance()->get_module(
                               "III-Matching") +
                               Util::Collector::get_instance()->get_module(
                                   "III-Batching")
                         : Util::Collector::get_instance()->get_module(
                               "III-Matching"));
