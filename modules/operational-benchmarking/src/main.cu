...
/**
 * @brief Creates target in the center of the trap array, filling the width of the array
 */

void create_rectangular_target( std::vector<int32_t> &target_config, int num_trap, int num_target, int Nt_x, int Nt_y){
...
int main(int argc, char * argv[]) {
    if (argc != 4) {
        std::cout << "Usage is operational_benchmarking <problem_file_path> <num_trials> <num_repititions> " << std::endl;
        return 1;
    }
    ...
    //creates a symmetrical rectangular target configuration, filling the width of the trap array

    create_rectangular_target(target_config, Nt_x * Nt_y, num_target, Nt_x, Nt_y);
    ...
    // Number of implantation or extraction moves, used for debugging the batching
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        ...
        for(int rep = 0; rep < num_reps; ++rep) {
            ...
            while (!Util::target_met(rep_config, target_config)) {
                ...
                TrapArray trap_array = TrapArray(Nt_x, Nt_y, rep_config, loss_params_alpha , loss_params_nu , lifetime);  
                ...
                // Output the moves list and count number of extraction/implantation
                 ...
                 for(auto i:moves_list){
                    std::cout<< std::get<0>(i)<<" x " <<std::get<1>(i)<<" y "<<std::get<2>(i)<<" b "<<std::get<3>(i)<<std::endl;
