...
struct RamseyStabilizer60HzModelParameters {
    ...
    // Integral from t=0 to t=t0: A * T_A * (1 - exp(-t/T_A)) + B * T_B * (1 - exp(-t/T_B)) + a1/(2 pi nu_AC) sin(2 pi nu_AC (t+dt)) + a2/(2 pi 3 nu_AC) sin(2 pi 3 nu_AC (t+dt)) + b t
};
...
class RamseyStabilizer60HzModel {
    ...
    public:
        ...
        inline double get_phase_correction(double t) const {
            return 2 * M_PI * (this->parameters.A * this->parameters.T_A * (1 - std::exp(-t / this->parameters.T_A)) + 
                   this->parameters.B * this->parameters.T_B * (1 - std::exp(-t / this->parameters.T_B)) + 
