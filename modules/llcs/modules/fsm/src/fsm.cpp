/**
 * @brief   Class that implements a programmable finite state machine for execution of low latency procedures.
 * @date    Oct 2023
 */
 ...
 * 
 * Construct state objects for all states in the FSM. Connect the state transitions between these states. 
 */
template<typename AWG_T>FiniteStateMachine<AWG_T>::FiniteStateMachine( Server *s, TriggerDetector<AWG_T> *td ){
    std::cout << "FSM:: constructor" << std::endl;

    // 1. Create the ST_FAULT state
    State *fault_state = new State([this](){ this->st_FAULT();}, [](){ return -1; }, NULL );    
...
/**
 * @brief The member function that creates all state objects for the states of the non configurable part of the FSM .
 * 
...
template<typename AWG_T>void FiniteStateMachine<AWG_T>::setupFSM(){
    ...
    State *begin           = new State( [this](){ this->st_BEGIN(); }, [](){ return 1; }, f);
    State *idle_state      = new State( [this](){ this->st_IDLE(); },  [this](){ return server->listen(); }, f );
    State *config_hw_state = new State( [this](){ this->st_CONFIG_HW(); },  [](){ return 1; }, f ); 
    State *config_sm_state = new State( [this](){ this->st_CONFIG_SM(); },  [](){ return 1; }, f ); 
    State *ready_state     = new State( [this](){ this->st_READY(); },   [this](){  int t; while(true){t = trigger_detector->detectTrigger(6000); if(t != NO_HW_TRIG){ return numExperiments; } return -1;}}, f );
    State *trigger_done_state = new State( [this](){ this->st_TRIGGER_DONE(); },  [](){ return 1; }, f );
    State *last_trigger_done_state = new State( [this](){ this->st_LAST_TRIGGER_DONE(); },  [](){ return 1; }, f );
    State *reset_state = new State( [this](){ this->st_RESET(); }, [](){ return 1; }, f);
    State *close_awg = new State( [this](){ this->st_CLOSE_AWG(); }, [this](){ return server->listen(); }, f);
    State *restart_awg = new State( [this](){ this->st_RESTART_AWG(); }, [](){ return 1; }, f);
    State *exit_state      = new State( [this](){ this->st_EXIT(); },  [](){ return 1; }, NULL );
 ...
 * 
 * Create a state object for every action function that should run during an experimental shot.
 * Adds this state object to the list of state objects in the fsm. 
 * 
 */
template<typename AWG_T>void FiniteStateMachine<AWG_T>::programStates(std::string filepath){
    ...
    std::map<std::string, std::vector<ModuleType>> data = H5Wrapper::parseSMConfig(filepath);
    ...
    for( const auto& pair : data ){
        ...
        trig_num = std::stoi(str_trig_num);         // Assume this doesn't error, can add error checking later

        // Push new states in order into temporary list
        for( ModuleType val : execution_sequence ){
            std::function<void()> action_function = dyn_state_action_func_map[val];

            state_to_add = new State( action_function, [trig_num](){ return trig_num; }, fault_transition ); // NOTE: transition may be removed, superfluous for now
            switch(val){
                case M_LLRS:
        ...
        for( size_t i = 0; i < temp.size(); i++ ){
            ...
            if( i + 1 < temp.size() ){
                ...
                //transition to the ready state to await hardware trigger if this is not the last sequence
                if( (size_t)trig_num != num_keys){

                    //transition to the done shot state at the end of every sequence
                    s->addStateTransition( trig_num, trigger_done_state );
                    std::cout<<" to "<<s->getNextProgrammableState(trig_num)->getType();
                    // prevents the transition vector for done from getting infinitely large. Used <= since there will always be a fault transition
                    if(s->getNextProgrammableState(trig_num)->getTransitionSize() <= trig_num){
                        s->getNextProgrammableState(trig_num)->addStateTransition( trig_num, ready_state );
                        std::cout<<" to "<<s->getNextProgrammableState(trig_num)->getNextProgrammableState(trig_num)->getType();
                    }
                    std::cout<<std::endl;
                }
                //transition to the idle state after the last sequence
                else{
                    //transition to the done shot 2 state at the end of the experiment
                    s->addStateTransition( trig_num, last_trigger_done_state );
                    std::cout<<" to "<<s->getNextProgrammableState(trig_num)->getType();
                    // prevents the transition map for done 2 from getting infinitely large.
                    if(s->getNextProgrammableState(trig_num)->getTransitionSize() == 1){                    
                        s->getNextProgrammableState(trig_num)->addStateTransition( 1, idle_state );
                        std::cout<<" to "<<s->getNextProgrammableState(trig_num)->getNextProgrammableState(1)->getType();
                    }
                    std::cout<<std::endl;
                }
            }
        ...
        // add the state transition from the ST_READY state to the new sequence of states
...
template<typename AWG_T>void FiniteStateMachine<AWG_T>::saveMetadata(std::string dirPath){
    json json_data;
    ...
    for (int i = 0; i < llrs_metadata.size(); i++) {
        filePath = dirPath + "metadata_" + std::to_string(i) + ".json"; // FIX THIS
        ...
        for (int cycle_index = 0; cycle_index < numCycles; ++cycle_index) {
            ...
            for (const auto& move : movesPerCycle[cycle_index]) {
                jsonMoves.push_back({std::get<0>(move), std::get<1>(move), std::get<2>(move), std::get<3>(move), std::get<4>(move)});
            }
            cycle_data["moves"] = jsonMoves;
            cycle_data["final_atom_configuration"] = atomConfigs[cycle_index + 1];
 ...
 * 
 * Begin with the idle state and execute each state and perform the appropriate state transition.
 */
...
template<typename AWG_T>void FiniteStateMachine<AWG_T>::st_LAST_TRIGGER_DONE(){
