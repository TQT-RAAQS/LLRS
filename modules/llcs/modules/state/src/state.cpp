 ...
 * 
 */
State::State( std::function<void()> ac_func, std::function<int()> tr_func, State *fault_transition ){
...
void State::addStateTransition( int data, State *state_transition ){
    ...
    transitions.insert( transitions.end() - 1, std::make_tuple( data, state_transition ));
...
State* State::getNextProgrammableState(int result){
    ...
    // No transition picked, return fault transtions, in principle, this should'nt happen becase 
    // we should be returning a faulty value if the transition fails
...
void State::printState(){
    ...
    for( auto &transition: transitions ){
        std::cout << "      Transition Data  -> " << std::get<0>(transition) << std::endl;
        std::cout << "      State Transition -> " << (std::get<1>(transition)) << std::endl;
       
        if(std::get<1>(transition)!=nullptr){
            std::cout << "      Next State Type  -> "<< (std::get<1>(transition))->getType() << std::endl; 
        }
