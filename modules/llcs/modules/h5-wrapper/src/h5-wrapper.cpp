...
herr_t visit_groups(hid_t loc_id, const char *name, const H5L_info_t *info,
                    void *operator_data) {
...
H5Wrapper::parseSMConfig(std::string filepath) {
    ...
    std::map<std::string, ModuleType> stringToModuleType = {
        {"LLRS", M_LLRS}, {"CLO", M_CLO}, {"RYDBERG", M_RYDBERG}};
    ...
    H5Literate(FSM_config_group.getId(), H5_INDEX_NAME, H5_ITER_NATIVE, NULL,
               visit_groups, &group_names);
...
std::string H5Wrapper::convertHWConfig(std::string filepath) {
    ...
    std::string yamlFilepath = std::string(getenv("HOME")) +
                               "/Experiment/experiment/modules/LLRS/config/" +
                               ymlfilename + ".yml";
    ...
    H5Literate(pGroup.getId(), H5_INDEX_NAME, H5_ITER_NATIVE, NULL,
               visit_groups, &group_names);

    emitter << YAML::Key << "problem_definition" << YAML::Value
            << YAML::BeginMap;
    for (auto group_name : group_names) {
        Group group2 =
            file.openGroup("/devices/LLRS/problem_definition/" + group_name);
