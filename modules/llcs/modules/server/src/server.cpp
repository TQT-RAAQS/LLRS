...
Server::Server(){
    ...
    // set a 50 second timeout on socket.recv() FS: not sure if this is necessary
...
void replaceStr(std::string &requestStr, std::string toReplace, std::string targetStr){
...
std::string Server::get_metadata_file_path(){
    return metadata_file_path;
}

std::string Server::get_config_file_path(){
    ...
    try {
        ...
        if (requestStr == "hello") {
            ...
            if (json_file.is_open()) {
                ...
                json_file.close();
                std::string statusMsg = "LLRS reconfig saved to " + llrs_reset_filepath;
                zmq::message_t status(statusMsg.size());
                memcpy(status.data(), statusMsg.data(), statusMsg.size());
                socket.send(status);
                transition = 6;
            } else {
                std::string statusMsg = "Error opening file: " + llrs_reset_filepath;
                zmq::message_t status(statusMsg.size());
                memcpy(status.data(), statusMsg.data(), statusMsg.size());
                socket.send(status);
                transition = -1;
            ...
            std::cerr << "Invalid message type received: " << requestStr << std::endl;
            transition = -1;
        }
