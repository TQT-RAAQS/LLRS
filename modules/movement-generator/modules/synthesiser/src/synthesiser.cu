...
Synthesiser::Move Synthesiser::process_move(MovementsConfig movementsConfig, int i) {
...
std::vector<short> Synthesiser::synthesise(Move move) {
	switch (move.func_type) {
		case SIN_STATIC: 
			Synthesis::Waveform::set_static_function(std::make_unique<Synthesis::Sin>());
			break;
		case ERF_TRANSITION:
			Synthesis::Waveform::set_transition_function(std::make_unique<Synthesis::ERF>(move.duration, move.vmax));
			break;
		case TANH_TRANSITION:
			Synthesis::Waveform::set_transition_function(std::make_unique<Synthesis::TANH>(move.duration, move.vmax));
			break;
		case SPLINE_TRANSITION:
			Synthesis::Waveform::set_transition_function(std::make_unique<Synthesis::Spline>(move.duration));
			break;
	} 
	...
	if (move.type == FORWARD || move.type == BACKWARD) {
		d_nu_1 = move.index?
			std::get<0>(coef_x.at(move.index)) - std::get<0>(coef_x.at(move.index - 1)):
			std::get<0>(coef_x.at(move.index + 1)) - std::get<0>(coef_x.at(move.index));
		d_nu_1 = d_nu_1 / 2;
		d_nu_2 = move.index + move.block_size != coef_x.size()?
			std::get<0>(coef_x.at(move.index + move.block_size + 1)) - std::get<0>(coef_x.at(move.index + move.block_size)):
			std::get<0>(coef_x.at(move.index + move.block_size)) - std::get<0>(coef_x.at(move.index + move.block_size - 1));
		d_nu_2 = d_nu_2 / 2;
...
void Synthesiser::synthesise_and_upload(AWG &awg) {
    ...
    for (int i = 0; i < moves.size(); i++){
        AWG::TransferBuffer buffer = awg.allocate_transfer_buffer(static_cast<int>(moves[i].duration * sample_rate), false);
        std::vector<short> waveform = synthesise(moves[i]);
        memcpy(*buffer, waveform.data(), static_cast<int>(moves[i].duration * sample_rate * sizeof(short)));
        awg.init_and_load_range(*buffer, static_cast<int>(moves[i].duration * sample_rate), i, i + 1);
    } 
