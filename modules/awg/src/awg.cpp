...
uint32 AWG::read_config(std::string filename){
    ...
    config.samples_per_segment     = config.waveforms_per_segment * config.waveform_length;
...
uint32 AWG::configure(){
    if (read_config(AWG_CONFIG_PATH) != AWG_OK) {
        std::cout << "Error occured in parsing AWG config.\n";
    }
    ...
    set_dout_trigger_mode(SPCM_X1_MODE, SPCM_XMODE_DIGOUTSRC_CH1);     /// x1 - take image
    ...
    if (_max_seg == 0 || ((_max_seg & (_max_seg - 1)) != 0) || _max_seg > _max_step) {
        std::cerr << "ERROR: StreamSequence Constructor -> max_seg needs to be a power of 2 and less than " << _max_step << std::endl;
        return AWG_ERR;
    }
    ...
    const int64_t samples_per_segment_max   = total_sample_capacity / awg_total_segments;
    assert(waveforms_per_segment * samples_per_waveform <= samples_per_segment_max);
    ...
    for (size_t i = 0; i < channels.size(); ++i) {
        switch (channels[i]) {
            ...
            case 3:
                ...
                break;
            default:
                std::cout << "Channel not supported\n";
                break;
        }
    }
 ...
 * @return Error code 
*/
uint32 AWG::enable_outputs(const std::vector<int>& channels, const std::vector<int>& amp) {
...
/**
 * @brief Set MPIO line as SYNC digital output with the analog channels. Digital logical 1 is set for each sample as the most significant bit.
 * @return Error code
*/
uint32 AWG::set_dout_trigger_mode(int32 line, int32 channel) {
    int trigger_mode = (SPCM_XMODE_DIGOUT | channel | SPCM_XMODE_DIGOUTSRC_BIT15);
    return spcm_dwSetParam_i32(this->p_card, line, trigger_mode);
}


/**
 * @brief Set MPIO line as ASYNCOUT for external triggers. M2CMD_CARD_START needs to be called after a change in setting.
 * @return Error code
*/
...
/**
 * @brief Async pulse is generated on all MPIOs that are set as SPMCM_XMODE_ASYNCOUT
 * @return Error code
*/
...
uint32 AWG::start_stream() {
    return spcm_dwSetParam_i32(this->p_card, SPC_M2CMD, M2CMD_CARD_START | M2CMD_CARD_FORCETRIGGER);
}
...
/**
* @brief: Initialize a step in AWG's sequence memory by combining all the parameters to one int64 bit value
...
* @param llCondition => end condition (SPCSEQ_ENDLOOPALWAYS, SPCSEQ_ENDLOOPONTRIG, SPCSEQ_END) 
* @return error code
*/
uint32 AWG::seqmem_update(int64 lStep, int64 llSegment, int64 llLoop, int64 llNext, uint64 llCondition){
    uint64 llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment);
...
uint32 AWG::load_data(uint32 seg_num, short* p_data, uint64 size){
    ...
    #ifdef ENABLE_CUDA
        spcm_dwDefTransfer_i64(p_card, SPCM_BUF_DATA, SPCM_DIR_GPUTOCARD, 0, p_data, 0, size);
    #else
        spcm_dwDefTransfer_i64(p_card, SPCM_BUF_DATA, SPCM_DIR_PCTOCARD, 0, p_data, 0, size);
    #endif

    return spcm_dwSetParam_i32(p_card, SPC_M2CMD, M2CMD_DATA_STARTDMA|M2CMD_DATA_WAITDMA);
}
...
uint32 AWG::load_data_async_start(uint32 seg_num, short* p_data, uint64 size){
    ...
    spcm_dwDefTransfer_i64 (p_card, SPCM_BUF_DATA, SPCM_DIR_GPUTOCARD, 0, p_data, 0, size);
#else
    spcm_dwDefTransfer_i64(p_card, SPCM_BUF_DATA, SPCM_DIR_PCTOCARD, 0, p_data, 0, dwSegLenByte);  // DKEA: changed this, need to verify correctness
#endif
...
uint32 AWG::init_segment(uint32 seg_num, int num_samples) {
    spcm_dwSetParam_i32(p_card, SPC_SEQMODE_WRITESEGMENT,  seg_num);
    if (spcm_dwSetParam_i32(p_card, SPC_SEQMODE_SEGMENTSIZE, num_samples) != ERR_OK){
        std::cerr << "ERROR: AWG Sequence -- failed to initialize segment = " << seg_num << std::endl;
        print_awg_error();
        return AWG_ERR;
    }
    return AWG_OK;
...
/**
* @brief: initialize all AWG data memory segments and upload one given set of samples to all of them
* @param p_segment => pointer to array containing samples (16bit integers)
* @param num_samples => number of samples from p_segment to load, starting from first index
* @return status code
*/
...
/**
* @brief: initialize a range of the form [start, end) of AWG data memory segments and upload one 
*   given set of samples to all of them
* @param p_segment => pointer to array containing samples (16bit integers)
* @param num_samples => number of samples from p_segment to load, starting from first index
...
* @return status code
*/
uint32 AWG::init_and_load_range(short * p_segment, int num_samples, int start, int end){
    int status;
    int dwSegLenByte = bps * dwFactor * num_samples * lSetChannels;
...
* @return the step at which the error was generated, i.e. the step that is streaming in the sequence memory
*/
uint32 AWG::get_current_step() {
    int32 current_step = INT_MAX;
    if (spcm_dwGetParam_i32(p_card, SPC_SEQMODE_STATUS, &current_step) != ERR_OK) {
        std::cerr << "ERROR: AWG Sequence -- failed to get current step." << std::endl;
        print_awg_error();
    }
