...
typedef struct {
    ...
    int32
        lModulesCount; // number of installed modules for data sorting algorithm
    ...
    union {
        ...
        struct ST_SPCM_AI {
            ...
            int32 lMaxADCValue; // maximum range, normally 2^(Resolution-1) but
                                // can be limited
            ...
            int32 lSetRange[SPCM_MAX_AICHANNEL]; // current used input range for
                                                 // each channel
        ...
        struct ST_SPCM_AO {
            int32 lResolution;  // resolution of analog channels
            int32 lMaxDACValue; // maximum range, normally 2^(Resolution-1) but
                                // can be limited
        ...
        struct ST_SPCM_DIO {
            int32
                lGroups; // number of channel groups that have individual setup
            bool bInputTermAvailable;      // input termination available
            bool bDiffModeAvailable;       // differential mode available
            bool bStopLevelProgrammable;   // programmable stop level available
            bool bOutputLevelProgrammable; // low and high output level is
                                           // programmable
        } stDIO;
...
bool bSpcMInitCardByIdx( // returns false if error occured, otherwise true
    ST_SPCM_CARDINFO
        *pstCardInfo, // pointer to an allocated and empty card info structure
    int32 lCardIdx);  // index of card to open, index starts with zero

bool bSpcMInitCardByIdx( // returns false if error occured, otherwise true
    ST_SPCM_CARDINFO
        *pstCardInfo, // pointer to an allocated and empty card info structure
    const char *szDrvName, // name of card like /dev/spcm0 (local) or
                           // 123.123.123.123 (for digitizerNETBOX only)
    int32 lCardIdx);       // index of card to open, index starts with zero
...
char *pszSpcMPrintDocumentationLink(
    const ST_SPCM_CARDINFO
        *pstCardInfo, // pointer to a filled card info structure
    char *pszBuffer,  // buffer for printing
    int32 lStrLen);   // length of the buffer
...
bool bSpcMSetupModeRecStdSingle(   // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
...
bool bSpcMSetupModeRecFIFOSingle(  // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable, // channel enable mask for the next acquisition
    int64
        llPreSamples, // number of samples to be stored before the trigger event
    int64 llBlockToRec =
        0, // blocks and loops can define the maximum recording length
    int64 llLoopToRec =
        0); // in FIFO mode as Block * Loop. If zero we run continuously

// ***********************************************************************

// record standard mode average
bool bSpcMSetupModeRecStdAverage(  // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,   // channel enable mask for the next acquisition
    int64 llMemSamples,  // recording length in samples per channel
    ...
    int64
        llPostSamples, // samples to record after trigger event for each segment
    int32 lAverages    // number of triggered segments to average
);

// record standard mode multiple recording
bool bSpcMSetupModeRecStdMulti(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,    // channel enable mask for the next acquisition
    int64 llMemSamples,   // recording length in samples per channel
    int64 llSegmentSize,  // size of each multiple recording segment
    int64 llPostSamples); // samples to record after trigger event for each
                          // segment
...
bool bSpcMSetupModeRecFIFOAverage( // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,   // channel enable mask for the next acquisition
    int64 llSegmentSize, // size of each multiple recording segment
    int64
        llPostSamples, // samples to record after trigger event for each segment
    int32 lAverages,
    int64 llSegmentsToRec = 0); // number of segments to record in total. If
                                // zero we reun continuously

// record FIFO mode multiple recording
bool bSpcMSetupModeRecFIFOMulti(   // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,   // channel enable mask for the next acquisition
    int64 llSegmentSize, // size of each multiple recording segment
    int64
        llPostSamples, // samples to record after trigger event for each segment
    int64 llSegmentsToRec = 0); // numbe of segments to record in total. If zero
                                // we reun continuously
...
void *
pvGetSegmentDataPointer( // returns an pointer to the segment start address
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    void *pvDataBuffer, // pointer to the data array that holds all segments
    int32 lSegmentsize, // size of one segment
    int32
        lSegmentIdx, // index of the segment of which we wish to get the pointer
...
bool bSpcMSetupModeRecFIFOGate(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,       // channel enable mask for the next acquisition
    int64 llPreSamples,      // number of samples to record before gate starts
...
bool bSpcMSetupModeRepStdSingle(   // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
...
bool bSpcMSetupModeRepStdLoops(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,  // channel enable mask for the next generation
    int64 llMemSamples, // samples to replay after trigger event
...
bool bSpcMSetupModeRepStdSingleRestart( // returns false if error occured,
                                        // otherwise true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,  // channel enable mask for the next generation
    int64 llMemSamples, // samples to replay after trigger event
...
bool bSpcMSetupModeRepFIFOSingle(  // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable, // channel enable mask for the next acquisition
    int64 llBlockToRep =
        0, // blocks and loops can define the maximum replay length
    int64 llLoopToRep =
        0); // in FIFO mode as Block * Loop. If zero we run continuously
...
bool bSpcMSetupModeRepStdMulti(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,          // channel enable mask for the next generation
    int64 llMemSamples,         // replay length in samples per channel
    int64 llSegmentSize,        // size of each segment
    int64 llSegmentsToRep = 1); // segments to replay (0 = infinite, 1 = memsize
                                // once, N = number of segments)

// FIFO mode multiple replay
bool bSpcMSetupModeRepFIFOMulti(   // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,   // channel enable mask for the next generation
    int64 llSegmentSize, // size of each segment
    int64 llSegmentsToRep =
        0); // segments to replay (0 = infinite, N = number of segments)
...
bool bSpcMSetupModeRepStdGate( // returns false if error occured, otherwise true
    ...
    uint64 qwChEnable,       // channel enable mask for the next generation
    int64 llMemSamples,      // recording length in samples per channel
    int64 llGatesToRep = 1); // gates to replay (0 = infinte, 1 = memsize once,
                             // N = number of gates)

// FIFO mode gated replay
bool bSpcMSetupModeRepFIFOGate(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable, // channel enable mask for the next generation
    int64 llGatesToRep =
        0); // gates to replay (0 = infinte, N = number of gates)

// ***********************************************************************

bool bSpcMSetupModeRepSequence(    // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    uint64 qwChEnable,     // channel enable mask for the next generation
...
bool bSpcMSetupTrigExternal( // returns false if error occured, otherwise true
    ...
    int32 lPulsewidth =
        0, // programmable pulsewidth for all external + pulsewidth modes
    bool bSingleSrc =
        true, // acts as single trigger source, all other masks cleared
    int32 lExtLine = 0); // standard external trigger is line 0

// external analog trigger with programmable levels
bool bSpcMSetupTrigExternalLevel(  // returns false if error occured, otherwise
                                   // true
    ST_SPCM_CARDINFO *pstCardInfo, // pointer to a filled card info structure
    int32 lExtMode,                // external trigger mode
...
// this function sets the trigger masks (bSingleSrc of other commands must be
// false to use this)
...
bool bSpcMSetupAnalogOutputChannel( // returns false if error occured, otherwise
                                    // true
    ST_SPCM_CARDINFO *pstCardInfo,  // pointer to a filled card info structure
    int32 lChannel,                 // channel to change
    int32 lAmplitude, // output amplitude in mV = (max-min)/2, =1000 for +/-1V
                      // range
    int32 lOutputOffset =
        0,             // programmable output offset as listed in the manual
    int32 lFilter = 0, // programmable output filter as listed in the manual
    int32 lStopMode = SPCM_STOPLVL_ZERO, // defines the behavior after replay or
                                         // while replay is pausing
    bool bDoubleOut = false, // enables identical    output on two channels of
                             // one module (if available)
    bool bDifferential = false); // enables differential output on two channels
                                 // of one module (if available)
...
bool bSpcMSetupDigitalOutput( // returns false if error occured, otherwise true
    ...
    int32 lStopMode = SPCM_STOPLVL_LOW, // defines the behavior after replay or
                                        // while replay is pausing
    int32 lLowLevel = 0, // low level in mV if output is programmable
    int32 lHighLevel =
        3300, // high level in mV if output levels are programmable
...
bool bSpcMSetupTimestamp( // returns false if error occured, otherwise true
    ...
    uint32 dwRefTimeoutMS); // timeout in milli seconds for synchronisation with
                            // reference clock
