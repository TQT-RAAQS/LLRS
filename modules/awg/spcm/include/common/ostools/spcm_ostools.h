...
typedef SPCM_THREAD_RETURN(SPCM_THREAD_CALLTYPE SPCM_THREADFUNCTION)(
    void *pvArguments);

// define the thread priority settings
enum SPCM_THREADPRIO { ePrioMin, ePrioNormal, ePrioMax };

// creates a thread with the given function name, returns false if creation
// failed
...
// wait for one event with timeout, true if event was received, false if timeout
// occurs, timeout zero means that we wait forever
bool spcm_bWaitEventWithMutex(SPCM_EVENT_HANDLE *phEvent,
                              SPCM_MUTEX_HANDLE *phMutex,
                              uint32 dwTimeoutMS = 0);
