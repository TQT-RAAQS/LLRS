    ...
    T **ppTChannelData)            // generic array of pointers for demuxed data
{
    ...
    if (pstCardInfo->qwSetChEnableMap &
        ...
        for (lCh = 0; lCh < (pstCardInfo->lSetChannels >> 1); lCh++) {
            ...
            ppTChPtr[2 * lCh + 1] =
                ppTChannelData[(pstCardInfo->lSetChannels >> 1) + lCh];
        }
...
bool bSpcMCalcSignal(              // return true if calculation has succeeded
    ...
    uint32 dwByteWidth = 0, // bytes width (for digital cards), if zero we
                            // expect a matching width to analog resolution
