...
bool bSpcMDemuxDigitalInputDataToUInt8(ST_SPCM_CARDINFO *pstCardInfo,
                                       void *pvMuxData, uint32 dwLenInSamples,
                                       uint8 **ppbyData) {
    uint32 dwSample;
    int32 lCh;
    ...
    if (pstCardInfo->qwSetChEnableMap &
        ~((1 << (pstCardInfo->lMaxChannels / pstCardInfo->lModulesCount)) - 1))
        for (lCh = 0; lCh < (pstCardInfo->lSetChannels >> 1); lCh++) {
            ppbyChPtr[2 * lCh + 0] = ppbyData[lCh];
            ppbyChPtr[2 * lCh + 1] =
                ppbyData[(pstCardInfo->lSetChannels >> 1) + lCh];
        }
...
bool bSpcMDemuxDigitalDataToInt8(ST_SPCM_CARDINFO *pstCardInfo, void *pvMuxData,
                                 uint32 dwLenInSamples, int8 **ppbyData) {
    int16 nGroupSample;
    int8 bySample;
    ...
    // split data
    for (dwGroupIdx = 0;
         dwGroupIdx < (dwLenInSamples * pstCardInfo->lSetChannels) / 16;
         dwGroupIdx++) {
        ...
        case 8:
            for (lChIdx = 0; lChIdx < 16; lChIdx++) {
                ...
                ppbyData[lChIdx % pstCardInfo->lSetChannels][lSampleIdx] =
                    bySample;
            }
...
double dSpcMIntToVoltage(ST_SPCM_CARDINFO *pstCardInfo, int32 lChannel,
                         double dValue) {
    if (!pstCardInfo)
        return 0;
    ...
    if ((lChannel < 0) || (lChannel >= pstCardInfo->lMaxChannels)) {
        sprintf(pstCardInfo->szError,
                "SpcMIntToVoltage: channel number %d not valid. Channels range "
                "from 0 to %d\n",
    ...
    dVoltage_mv = pstCardInfo->uCfg.stAI.lSetRange[lChannel] * dValue /
                  pstCardInfo->uCfg.stAI.lMaxADCValue;

    // add the signal offset
    if (pstCardInfo->uCfg.stAI.bOffsPercentMode)
        dVoltage_mv -= pstCardInfo->uCfg.stAI.lSetRange[lChannel] *
                       pstCardInfo->uCfg.stAI.lSetOffset[lChannel] / 100;
    else
...
bool bSpcMDemuxAnalogDataToVoltage(ST_SPCM_CARDINFO *pstCardInfo,
                                   void *pvMuxData, uint32 dwLenInSamples,
                                   float **ppfData) {
    uint32 dwSample;
    int32 lCh;
    ...
    if (pstCardInfo->qwSetChEnableMap &
        ...
        for (lCh = 0; lCh < (pstCardInfo->lSetChannels >> 1); lCh++) {
            ...
            ppfChPtr[2 * lCh + 1] =
                ppfData[(pstCardInfo->lSetChannels >> 1) + lCh];
        }

    // calculate offset and factor for re-calculation to voltage
    for (lCh = 0; lCh < pstCardInfo->lSetChannels; lCh++) {
        dFactor[lCh] = (double)pstCardInfo->uCfg.stAI.lSetRange[lCh] /
                       pstCardInfo->uCfg.stAI.lMaxADCValue / 1000.0;

        // add the signal offset
        if (pstCardInfo->uCfg.stAI.bOffsPercentMode)
            dOffset[lCh] = -(double)pstCardInfo->uCfg.stAI.lSetRange[lCh] *
                           pstCardInfo->uCfg.stAI.lSetOffset[lCh] / 100.0 /
                           1000.0;
        else
            dOffset[lCh] =
                -(double)pstCardInfo->uCfg.stAI.lSetOffset[lCh] / 1000.0;
    }
    ...
    if (pstCardInfo->lBytesPerSample > 1) {
        ...
        for (dwSample = 0; dwSample < dwLenInSamples; dwSample++)
            for (lCh = 0; lCh < pstCardInfo->lSetChannels; lCh++)
                *ppfChPtr[lCh]++ =
                    (float)(dOffset[lCh] + dFactor[lCh] * *pnMuxBuf++);
    }
    ...
    // split byte data
        ...
        for (dwSample = 0; dwSample < dwLenInSamples; dwSample++)
            for (lCh = 0; lCh < pstCardInfo->lSetChannels; lCh++)
                *ppfChPtr[lCh]++ =
                    (float)(dOffset[lCh] + dFactor[lCh] * *pbyMuxBuf++);
    }
...
bool bSpcMMuxData(ST_SPCM_CARDINFO *pstCardInfo, void *pvMuxData,
                  uint32 dwLenInSamples, void **ppvData) {
    int32 lCh;
    ...
    if (pstCardInfo->bM2i) {
        ...
        // ...
        if (pstCardInfo->qwSetChEnableMap &
            ~((1 << (pstCardInfo->lMaxChannels / pstCardInfo->lModulesCount)) -
              1)) {
            for (lCh = 0; lCh < (pstCardInfo->lSetChannels >> 1); lCh++) {
                ppvChPtr[2 * lCh + 0] = ppvData[lCh];
                ppvChPtr[2 * lCh + 1] =
                    ppvData[(pstCardInfo->lSetChannels >> 1) + lCh];
            }
        }
    }
    ...
    uint32 dwBytesPerSumSample =
        (uint32)(pstCardInfo->lSetChannels * pstCardInfo->lBytesPerSample);
    uint32 dwBytesPerSample = pstCardInfo->lBytesPerSample;

    for (dwSample = 0; dwSample < dwLenInSamples; dwSample++)
        for (lCh = 0; lCh < pstCardInfo->lSetChannels; lCh++)
            memcpy(((uint8 *)pvMuxData) + dwSample * dwBytesPerSumSample +
                       lCh * dwBytesPerSample,
                   ((uint8 *)ppvChPtr[lCh]) + dwSample * dwBytesPerSample,
                   dwBytesPerSample);
...
bool bSpcMSplitAnalogAndDigitalData(ST_SPCM_CARDINFO *pstCardInfo,
                                    void *pvMergedData, uint32 dwLenInSamples,
                                    void *pvAnalogData, void *pvDigitalData) {
...
bool bFillSB5Header(ST_SPCM_CARDINFO *pstCard, ST_SB5HEAD *pstHeader,
                    const char *pszName) {
    char szSource[25];
    int i;
    ...
    if (!pstHeader->pdSourceFS || !pstHeader->pdYOffset ||
        !pstHeader->pdYScale || !pstHeader->plMuxIdx)
        return false;
    ...
    case AnalogIn:
        ...
        case 8:
            pstHeader->lSignalType |= SIGNAL_TYP_8BIT | SIGNAL_TYP_1BYTE;
            break;
        case 12:
            pstHeader->lSignalType |= SIGNAL_TYP_12BIT | SIGNAL_TYP_2BYTE;
            break;
        case 14:
            pstHeader->lSignalType |= SIGNAL_TYP_14BIT | SIGNAL_TYP_2BYTE;
            break;
        case 16:
            pstHeader->lSignalType |= SIGNAL_TYP_16BIT | SIGNAL_TYP_2BYTE;
            break;
        default:
            break;
        }
    ...
    case TYP_M2ISERIES:
        sprintf(szSource, "M2i.%04x sn %05d",
                pstCard->lCardType & TYP_VERSIONMASK, pstCard->lSerialNumber);
        break;
    case TYP_M2IEXPSERIES:
        sprintf(szSource, "M2i.%04x-Exp sn %05d",
                pstCard->lCardType & TYP_VERSIONMASK, pstCard->lSerialNumber);
        break;
    default:
        sprintf(szSource, "Unknown %04x sn %05d",
                pstCard->lCardType & TYP_VERSIONMASK, pstCard->lSerialNumber);
        break;
    }
    ...
    pstHeader->lSumSamples =
        (int32)pstCard->llSetMemsize * pstCard->lSetChannels;
    ...
    case AnalogIn:
        for (i = 0; i < pstCard->lSetChannels; i++) {
            ...
            pstHeader->pdYOffset[i] =
                -(double)pstCard->uCfg.stAI.lSetOffset[i] / 100 *
                pstCard->uCfg.stAI.lSetRange[i];
            pstHeader->pdYScale[i] = (double)pstCard->uCfg.stAI.lSetRange[i] /
                                     pstCard->uCfg.stAI.lMaxADCValue / 1000.0;
        }

        // if two modules are active data is sorted mod0ch0, mod1ch0, mod0ch1,
        // ...
        if (pstCard->qwSetChEnableMap &
            ~((1 << (pstCard->lMaxChannels / pstCard->lModulesCount)) - 1))
            for (i = 0; i < (pstCard->lSetChannels >> 1); i++) {
                pstHeader->plMuxIdx[2 * i + 0] = i;
                pstHeader->plMuxIdx[2 * i + 1] =
                    (pstCard->lSetChannels >> 1) + i;
            }
...
bool bSpcMCalcSignal(ST_SPCM_CARDINFO *pstCardInfo, void *pvData,
                     uint32 dwLenInSamples, uint32 dwByteWidth,
                     E_SPCM_SIGSHAPE eShape, uint32 dwLoops, uint32 dwGainP) {
    ...
    case DigitalIO:
        if (dwByteWidth == 0) {
            sprintf(pstCardInfo->szError,
                    "ByteWidth can't be zero for digital cards as we didn't "
                    "know how much channels are activated\n");
            return false;
        }
    ...
    for (i = 0; i < dwLenInSamples; i++) {
        ...
        case eInvertedTriangle:
            if (dwPosInBlock < dwBlockHalf)
                ...
                llValue = (int64)(llMaxFS - (dwPosInBlock - dwBlockHalf) *
                                                dSpan / dwBlockHalf);
            break;
        ...
        default:
        case 1:
            *pbyData++ = (int8)llValue;
            break;
        case 2:
            *pnData++ = (int16)llValue;
            break;
        case 4:
            *plData++ = (int32)llValue;
            break;
        case 8:
            *pllData++ = (int64)llValue;
            break;
        }
    } // end of for-loop
