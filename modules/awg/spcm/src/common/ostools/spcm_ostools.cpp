...
void *pvAllocMemPageAligned(uint64 qwBytes) {
    ...
    pvTmp =
        (void *)mmap(NULL, qwBytes, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
...
bool spcm_bCreateThread(SPCM_THREADFUNCTION *pfnThread,
                        SPCM_THREAD_HANDLE *phThread, void *pvArguments) {
...
void spcm_vSetThreadPriority(SPCM_THREAD_HANDLE *phThread,
                             SPCM_THREADPRIO ePriority) {
...
void spcm_vSuspendThread(uint32) { pthread_yield(); }
...
bool spcm_bWaitEventWithMutex(SPCM_EVENT_HANDLE *phEvent,
                              SPCM_MUTEX_HANDLE *phMutex, uint32 dwTimeoutMS) {
    struct timespec ts;
    struct timeval tp;
    bool bRet;
    ...
    if (dwTimeoutMS) {
        ...
        ts.tv_nsec =
            (((ts.tv_nsec / 1000 / 1000) + dwTimeoutMS) % 1000) * 1000 * 1000;
...
void spcm_vGetMutex(SPCM_MUTEX_HANDLE *phMutex) { pthread_mutex_lock(phMutex); }
...
uint64 qwGetTotalVirtualMemory() {
    ...
    return (uint64)8 * 1024 * 1024 * 1024 *
           1024; // 8TB, value taken from 64-Bit Windows
#else            // 32 bit
    return (uint64)3 * 1024 * 1024 *
           1024; // 3GB for user-space in linux systems
#endif
}
} // namespace SPCM_NAMESPACE
