...
//     cudaDeviceProp stCUDADeviceProp;
//     cudaGetDeviceProperties (&stCUDADeviceProp, lCUDADevIdx);
//     if ((strncmp (stCUDADeviceProp.name, "Quadro", 6) != 0) && (strncmp (stCUDADeviceProp.name, "Tesla", 5) != 0))
//         {
//         printf ("ERROR: found \"%s\", but RDMA requires a Quadro or Tesla card.\n", stCUDADeviceProp.name);
//         return NULL;
//         }
//     printf("\nUsing device %d: \"%s\"\n", 0, stCUDADeviceProp.name);
...
//         }
    ...
    /*
    eCudaErr = cuMemAlloc((CUdeviceptr *)&pvDMABuffer_gpu, qwDMABufferSize_bytes);
    if (eCudaErr != cudaSuccess)
        {
        printf ("ERROR in cudaMalloc(): %s\n", cudaGetErrorString(eCudaErr));
        return NULL;
    ...
    CUresult eResult = cuPointerSetAttribute ((void*)&dwFlag, (CUpointer_attribute_enum)CU_POINTER_ATTRIBUTE_SYNC_MEMOPS, (CUdeviceptr)pvDMABuffer_gpu);
