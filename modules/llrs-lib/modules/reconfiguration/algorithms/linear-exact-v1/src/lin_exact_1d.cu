...
void compute_heights(int *initial, int *target, int num_traps, int heights[],
                     int *excess) {
...
// matching generator
void lin_exact_generate_matching(int *initial, int *target, int num_traps,
                                 int initial_atom_count, int target_atom_count,
                                 int matching_src[], int matching_dst[]) {
    int X = num_traps + 1;
    // Including 0; heights[0] = 0 and heights[num_traps + 1] = excess
    int heights[num_traps + 2];

    // Offset the initial just so that the sources and targets are in (0, X)
    // (set X to num_traps + 1)
    ...
    compute_heights(source_with_offset, target_with_offset, num_traps, heights,
                    &excess);
    ...
    // We can get the corresponding k using heights, the below arrays store the
    ...
    for (int i = 1; i <= num_traps; i++) {
        if (source_with_offset[i]) {
            S_tilde[heights[i] - h_min][S_tilde_indices[heights[i] - h_min]] =
                i;
        ...
        if (target_with_offset[i]) {
            T_tilde[heights[i] - h_min][T_tilde_indices[heights[i] - h_min]] =
                i;
            // Increment indices
    ...
    // pi contains the maximum profit needed for heights between 1 and excess
...
void generate_output_for_blocks_going_left(int matching_going_left_src[],
                                           int matching_going_left_dst[],
                                           int mgl_idx, int target_atom_count,
                                           int *src, int *dst, int *blk_sizes,
...
void generate_output_for_blocks_going_right(int matching_going_right_src[],
                                            int matching_going_right_dst[],
                                            int mgr_idx, int target_atom_count,
                                            int *src, int *dst, int *blk_sizes,
                                            int *batch_indices,
...
void block_output_generator_cpu(int matching_src[], int matching_dst[],
    ...
    *num_batches = 0;
    ...
    generate_output_for_blocks_going_right(
        matching_going_right_src, matching_going_right_dst, mgr_idx,
        target_atom_count, src, dst, blk_sizes, batch_indices, num_batches,
        sol_length);
    generate_output_for_blocks_going_left(
        matching_going_left_src, matching_going_left_dst, mgl_idx,
        target_atom_count, src, dst, blk_sizes, batch_indices, num_batches,
        sol_length);
}

void lin_exact_block_batched_cpu(int *initial, int *target, int num_traps,
                                 int inital_atom_count, int target_atom_count,
                                 int *src, int *dst, int *blk_sizes,
                                 int *batch_indices, int *num_batches,
                                 int *sol_length) {
    ...
    lin_exact_generate_matching(initial, target, num_traps, inital_atom_count,
                                target_atom_count, matching_src, matching_dst);

    block_output_generator_cpu(matching_src, matching_dst, target_atom_count,
                               src, dst, blk_sizes, batch_indices, num_batches,
                               sol_length);
}
...
void lin_exact_clump_batched_cpu(int *source, int *target, int N, int K,
