...
#include <stdio.h>
#include <stdlib.h>
void compute_heights_cpu(int *sourceFlags, int *targetFlags, int numTraps,
                         int *height) {
    int heightNow = 0;
...
void get_primes_cpu(int *sourceFlags, int *targetFlags, int *yPrime,
...
void compute_profits_cpu(int *sourceFlags, int numExcessSources, int *height,
                         int *max, int *max_index, int numTraps, int *yPrime,
                         int *profit) {
    ...
    for (int i = numTraps - 1; i > -1; i--) {
        if (sourceFlags[i] == 1) {
            if (height[i] > 0 && height[i] < (numExcessSources + 1)) {
                ...
                profit[i] =
                    2 * yPrime[i] - i - YDoublePrime + profit[YDoublePrime];
...
void lin_exact_cpu_v2_flags(int *inputSourceFlags, int *inputTargetFlags,
                            int numTraps, int numSources, int numTargets,
                            int *OutSourcesFlags) {
    ...
    get_primes_cpu(sourceFlags, targetFlags, yPrime, numTraps, numSources,
                   numTargets);
    ...
    compute_profits_cpu(sourceFlags, numExcessSources, height, max, max_index,
                        numTraps, yPrime, profit);
...
void lin_exact_cpu_v2_generate_matching(int *sourceFlags, int *targetFlags,
                                        int numTraps, int numSources,
                                        int numTargets, int *OutSources_cpu,
                                        int *OutTargets_cpu) {
    ...
    lin_exact_cpu_v2_flags(sourceFlags, targetFlags, numTraps, numSources,
                           numTargets, OutSourcesFlags);
...
void generate_output_for_blocks_going_left_v2(
    int matching_going_left_src[], int matching_going_left_dst[], int mgl_idx,
    ...
    for (int i = 0; i < mgl_idx; i++) {
        if ((i == mgl_idx - 1) || !(matching_going_left_src[i] ==
                                    matching_going_left_src[i + 1] - 1)) {
            is_head[i] = 1;
    ...
    for (int i = 0; i < mgl_idx; i++) {
        if ((i == 0) || !(matching_going_left_src[i] ==
                          matching_going_left_src[i - 1] + 1)) {
            block_to_tail[block_to_tail_index++] = i;
        }
    }

    // Now that we have the heads, do prefix sum to figure out the blocks for
    // each index
    ...
    // The number of batches going left is equal to the maximum distance going
    // left
    int max_distance_of_atoms_going_left = 0;

    for (int i = 0; i < mgl_idx; i++) {
        max_distance_of_atoms_going_left =
            max(max_distance_of_atoms_going_left,
                matching_going_left_src[i] - matching_going_left_dst[i]);
    }
    ...
    *num_batches += max_distance_of_atoms_going_left;
    ...
    for (int j = num_batches_going_right; j < *num_batches; j++) {
        ...
        for (int i = 0; i < mgl_idx; i++) {
            ...
            if (block_to_head[cur_block] == i &&
                matching_going_left_src[i] != matching_going_left_dst[i]) {
                // Head found, write to output
                blk_sizes[*sol_length] = i - block_to_tail[cur_block] + 1;
                src[*sol_length] =
                    matching_going_left_src[i] - blk_sizes[*sol_length] + 1;
                dst[*sol_length] =
                    matching_going_left_src[i] - blk_sizes[*sol_length];
                (*sol_length)++;
            }
        ...
        for (int i = 0; i < mgl_idx - 1; i++) {
            if (matching_going_left_src[i] != matching_going_left_dst[i] &&
                matching_going_left_src[i + 1] ==
                    matching_going_left_dst[i + 1] &&
                index_to_block[i] == index_to_block[i + 1]) {
                // Update head
...
void generate_output_for_blocks_going_right_v2(
    int matching_going_right_src[], int matching_going_right_dst[], int mgr_idx,
    int target_atom_count, int *src, int *dst, int *blk_sizes,
    int *batch_indices, int *num_batches, int *sol_length) {
    ...
    for (int i = 0; i < mgr_idx; i++) {
        if ((i == 0) || !(matching_going_right_src[i] ==
                          matching_going_right_src[i - 1] + 1)) {
            is_head[i] = 1;
            block_to_head[block_to_head_index++] = i;
        } else {
            is_head[i] = 0;
    ...
    // Do something similar for tails
    for (int i = 0; i < mgr_idx; i++) {
        if ((i == mgr_idx - 1) || !(matching_going_right_src[i] ==
                                    matching_going_right_src[i + 1] - 1)) {
            block_to_tail[block_to_tail_index++] = i;
        }
    }
    ...
    // The number of batches going right is equal to the maximum distance going
    // right
    int max_distance_of_atoms_going_right = 0;

    for (int i = 0; i < mgr_idx; i++) {
        max_distance_of_atoms_going_right =
            max(max_distance_of_atoms_going_right,
                matching_going_right_dst[i] - matching_going_right_src[i]);
    }
    ...
    *num_batches += max_distance_of_atoms_going_right;
    ...
    for (int j = 0; j < max_distance_of_atoms_going_right; j++) {
        ...
        for (int i = 0; i < mgr_idx; i++) {
            ...
            int cur_block = index_to_block[i];
            if (block_to_head[cur_block] == i &&
                matching_going_right_src[i] != matching_going_right_dst[i]) {
                // Head found, write to output
...
void lin_exact_1d_cpu_v2_block_output_generator(
    ...
    *num_batches = 0;
    ...
    // Split up the matching into two parts: things going right and things going
    // left
    ...
    generate_output_for_blocks_going_right_v2(
        matching_going_right_src, matching_going_right_dst, mgr_idx,
        target_atom_count, src, dst, blk_sizes, batch_indices, num_batches,
        sol_length);
    generate_output_for_blocks_going_left_v2(
        matching_going_left_src, matching_going_left_dst, mgl_idx,
        target_atom_count, src, dst, blk_sizes, batch_indices, num_batches,
        sol_length);
}

void clumped_block_output_generator_cpu(int matching_src[], int matching_dst[],
                                        int N, int K_prime, int *src_block,
                                        int *dst_block, int *size_block,
                                        int *batchPtr_block,
                                        int *num_batches_block,
                                        int *sol_length_block) {
    ...
    *num_batches_block = 0;
    ...
    // Generate the output by treating the target as the source, and the source
    // as the target
    ...
    generate_output_for_blocks_going_right_v2(
        matching_going_right_src, matching_going_right_dst, mgr_idx, K_prime,
        src_block_inv, dst_block_inv, size_block_inv, batchPtr_block_inv,
        &num_batches_block_inv, &sol_length_block_inv);
    generate_output_for_blocks_going_left_v2(
        matching_going_left_src, matching_going_left_dst, mgl_idx, K_prime,
        src_block_inv, dst_block_inv, size_block_inv, batchPtr_block_inv,
        &num_batches_block_inv, &sol_length_block_inv);
    ...
    // For things going left before the inversion, make sure to use the leftmost
    // atom as the head for (int i = 0; i < sol_length_block_inv; i++) {
    ...
    *sol_length_block = sol_length_block_inv;
    ...
    for (int i = num_batches_block_inv - 1; i >= 0; i--) {
        ...
        int batch_end = (i == num_batches_block_inv - 1)
                            ? sol_length_block_inv
                            : batchPtr_block_inv[i + 1];
...
void lin_exact_cpu_v2_block_batched(int *initial, int *target, int num_traps,
                                    int inital_atom_count,
                                    int target_atom_count, int *src, int *dst,
                                    int *blk_sizes, int *batch_indices,
                                    int *num_batches, int *sol_length) {

    int matching_src[target_atom_count];
    int matching_dst[target_atom_count];

    lin_exact_cpu_v2_generate_matching(initial, target, num_traps,
                                       inital_atom_count, target_atom_count,
                                       matching_src, matching_dst);

    lin_exact_1d_cpu_v2_block_output_generator(
        matching_src, matching_dst, target_atom_count, src, dst, blk_sizes,
        batch_indices, num_batches, sol_length);
}

void lin_exact_cpu_v2_clump_batched(int *source, int *target, int N, int K,
                                    int K_prime, int *src_c_block,
                                    int *dst_c_block, int *size_c_block,
                                    int *batchPtr_c_block,
                                    int *num_batches_c_block,
                                    int *sol_length_c_block) {

    int matching_src[K_prime];
    int matching_dst[K_prime];

    lin_exact_cpu_v2_generate_matching(source, target, N, K, K_prime,
                                       matching_src, matching_dst);

    // Do the flip here
    clumped_block_output_generator_cpu(matching_dst, matching_src, N, K_prime,
                                       src_c_block, dst_c_block, size_c_block,
                                       batchPtr_c_block, num_batches_c_block,
                                       sol_length_c_block);
}

void moves_generator_unbatched_cpu(int *OutSources, int *OutTargets,
                                   int numTargets, int *outputMovesSource,
                                   int *outputMovesTarget, int *sol_length) {
...
void lin_exact_cpu_v2_unbatched_moves(int *source, int *target, int N, int K,
                                      int K_prime, int *src_moves,
                                      int *dst_moves, int *sol_length) {
    ...
    moves_generator_unbatched_cpu(matching_src, matching_dst, K_prime,
                                  src_moves, dst_moves, sol_length);
}
