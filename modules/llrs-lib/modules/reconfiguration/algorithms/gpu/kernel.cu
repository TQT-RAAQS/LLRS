...
#define nextpow2(n) (n<=32)?32:(n<=64)?64:(n<=128)?128:(n<=256)?256:(n<=512)?512:1024

__device__ void compute_heights_gpu(int* buffer_s, int numTraps, int* heights_s){
    int* inBuffer_s = heights_s;
    int* outBuffer_s = buffer_s;
    for (unsigned int stride = 1; stride <= (nextpow2(numTraps)) / 2; stride *= 2){
        outBuffer_s[threadIdx.x] = inBuffer_s[threadIdx.x] + ((threadIdx.x >= stride)?(inBuffer_s[threadIdx.x - stride]):(0));
        __syncthreads();
        int* tmp = inBuffer_s;
...
__device__ void compressArray(int* buffer_s, int numTraps, int* flags_s, int* Output_d) {
	...
	if ((threadIdx.x == 0 && inBuffer_s[threadIdx.x] == 1) || (threadIdx.x != 0 && inBuffer_s[threadIdx.x] != inBuffer_s[threadIdx.x - 1])) {
		Output_d[inBuffer_s[threadIdx.x] - 1] = threadIdx.x;
	}
	
}

__device__ void linear_get_primes(int* sourceFlags_s, int* targetFlags_s, int numTraps, int* heights_s, int* y_prime_s){
		...
		while(i_forward<numTraps || i_backward>-1){
			...
			if(backward_flag == 0 && i_backward>-1){
				if (heights_s[i_backward] == height_x){
					...
					i_backward--;
					next_height_backward = abs(heights_s[i_backward]-height_x);
...
__device__ void compute_profits_gpu(int *buffer_s, int* sourceFlags_s,int numExcessSources, int* heights_s,int numTraps,int* y_prime_s, int* profit_s, KeyValue* max_profit_s){
	...
	while(flag == 1){
		...
		flag = 0;
		if(sourceFlags_s[threadIdx.x]==1){
			if(heights_s[threadIdx.x]<=numExcessSources && heights_s[threadIdx.x]>0){	
				if(next_source < numTraps-1){
					outBuffer_s[threadIdx.x] = inBuffer_s[threadIdx.x]+inBuffer_s[next_source];
					...
					if(entered == 0){
						...
						atomicMax( &((&max_profit_s[height_x-1])->combined), val.combined );
						entered = 1;
				}
				}
			}
...
__global__ void solve_gpu_kernel(int numExcessSources, int* sourceFlags, int* targetFlags, int numTraps, int* OutSources_gpu, int* OutTargets_gpu) {
	...
	if(blockIdx.x == 0){
		...
		heights_s[threadIdx.x] = sourceFlags_s[threadIdx.x] - ((threadIdx.x > 0)?(targetFlags_s[threadIdx.x - 1]):(0));
		...
		linear_get_primes(sourceFlags_s, targetFlags_s, numTraps, heights_s, y_prime_s);
		__syncthreads();
		
		compute_profits_gpu(buffer_s,sourceFlags_s,numExcessSources,heights_s,numTraps, y_prime_s, profit_s,max_profit_s);
		__syncthreads();
...
__global__ void populate_array_missing_sources(int* OutSources_gpu, int* targetFlags, int numTraps, int* OutTargets_gpu){
...
__global__ void populate_array_done(int numTraps, int* sourceFlags, int* OutSources_gpu, int* targetFlags, int* OutTargets_gpu){
...
__host__ void solve_gpu_d(int numExcessSources, int* sourceFlags, int* targetFlags, int numTraps, int* OutSources_gpu, int* OutTargets_gpu){

    if (numExcessSources > 0){
        solve_gpu_kernel<<<2, numTraps>>>(numExcessSources, sourceFlags, targetFlags, numTraps, OutSources_gpu, OutTargets_gpu);
    } else if (numExcessSources == 0){
        populate_array_done<<<2, numTraps>>>(numTraps, sourceFlags, OutSources_gpu, targetFlags, OutTargets_gpu);
    } else {
        populate_array_missing_sources<<<1, numTraps>>>(OutSources_gpu, targetFlags, numTraps, OutTargets_gpu);
	}
}