...
inline std::uint32_t Synthesis::get_key (Channel channel, WfType type, std::uint32_t index, std::uint32_t block_size, std::uint32_t extraction_extent) {

    return (channel << 30) + (type << 26) + (index << 18) + (block_size << 10) + extraction_extent;
}


void Synthesis::element_wise_add(std::vector<double> src, std::vector<double>& dst) {
...
Synthesis::WaveformRepo::WaveformRepo(std::size_t n_x, std::size_t n_y, double sample_rate, double wf_duration): _n_x(n_x), _n_y(n_y), _sample_rate(sample_rate), _wf_duration(wf_duration) {   
    _wf_len = (size_t)(_wf_duration * _sample_rate);
    // reserves enough space in repo to fit all waveforms according to repo dimensions
    _waveform_hashmap.reserve( (5 * _n_x * _n_x + _n_x)/2 + (5 * _n_y * _n_y + _n_y)/2 + 2 * (_n_x + _n_y - 2 ) );
}


void Synthesis::WaveformRepo::generate_repository(std::vector<WP> params_x, std::vector<WP>  params_y, double df) {
...
std::vector<double> Synthesis::WaveformRepo::get_waveform(Channel channel, WfType waveform_type,
std::uint32_t index, std::uint32_t block_size, std::uint32_t extraction_extent) const{
    int key = get_key(channel, waveform_type, index, block_size, extraction_extent);
    return _waveform_hashmap.at(key);
}

std::vector<double> Synthesis::WaveformRepo::get_waveform(std::uint32_t key) const
{
    return _waveform_hashmap.at(key);
}
...
int Synthesis::WaveformRepo::cache_waveform_repo(std::string fname) const
{
    ...
    if(!file.is_open()) return LLRS_ERR;
    ...
    for(const auto &data : _waveform_hashmap){
        ...
        for(const auto &val : data.second){
            ...
            if (!wpc.is_power_safe(data.second)) {
                ...
                std::cerr << "Power exceeded allowed threshold. Power = " << wpc.get_power_dBm(data.second) << 
                    "; Key=" << data.first << std::endl;
                throw std::runtime_error("Power exceeded the allowed threshold for a waveform when caching.");
            }
        #endif
    }
    ...
    while(file.read((char*)&key, sizeof(uint32_t))){
        ...
        for(int i = 0 ; i < _wf_len; i++){
            ...
            if (!wpc.is_power_safe(wf)) {
                file.close();
                std::cerr << "Power exceeded allowed threshold. Power = " << wpc.get_power_dBm(wf) << 
                    "; Key=" << key << std::endl;
                throw std::runtime_error("Power exceeded the allowed threshold for a waveform when loading.");
            }
        #endif
...
int Synthesis::WaveformRepo::cache_waveform_repo_plaintext(std::string fname) const {   
...
std::unordered_map<std::uint32_t, std::vector<double>> Synthesis::WaveformRepo::get_raw_map () const {
...
std::size_t Synthesis::WaveformRepo::get_dimension_x() {
    return this->_n_x;
}
std::size_t Synthesis::WaveformRepo::get_dimension_y() {
    return this->_n_y;
}
...
void Synthesis::WaveformRepo::generate_static_waveforms(Channel channel, std::vector<WP> params, std::size_t n) {
    ...
    /// generate every indivisual tone first and add it to temp vector and hashmap
    for(int i = 0; i < n; ++i) {
    ...
    /// use individual tones to generate block waveforms starting at start_idx and ending at end_idx

    for(int start_idx = 0; start_idx < n; start_idx++){

        /// we minimize summations by only keeping track of one sum and copying it to the map as we go
        ...
        for(int block_size = 2; start_idx + block_size - 1 < n; block_size++){
            ...
            _waveform_hashmap.emplace(get_key(channel, STATIC, start_idx, block_size, 0), 
                base);
        }
    }
...
void Synthesis::WaveformRepo::generate_displacement_waveforms(Channel channel, std::vector<WP> params, std::size_t n, double df) {
    ...
    for(int i = 0; i < n; i++){
        ...
        fade_in_forward[i]  = Displacement( params_before,  params[i] ).discretize(_wf_len, _sample_rate);
        fade_in_backward[i] = Displacement( params_after , params[i]  ).discretize(_wf_len, _sample_rate);

        fade_out_forward[i]  = Displacement( params[i], params_after  ).discretize(_wf_len, _sample_rate);
        fade_out_backward[i] = Displacement( params[i], params_before ).discretize(_wf_len, _sample_rate);
    }

    //generate individual moving displacement waveforms, add them to repo as special single tone displacements
    for(int i = 0; i < n - 1 ; i++){
        reverse_i =  n - 1 - i;
        move_forward[i]          = Displacement( params[i],         params[i+1]         ).discretize(_wf_len, _sample_rate);
        move_backward[reverse_i] = Displacement( params[reverse_i], params[reverse_i-1] ).discretize(_wf_len, _sample_rate);

        _waveform_hashmap.emplace(get_key(channel, RIGHTWARD, i, 1, 0),
                move_forward[i]);
        _waveform_hashmap.emplace(get_key(channel, LEFTWARD, reverse_i - 1, 1, 0),
                move_backward[reverse_i]);
    }

    //block waveform generation

    //temps are needed to avoid adding edge fading waveforms to intermediary sums
    ...
    for(int start_idx = 0; start_idx < n - 1; start_idx++){
        ...
        for(int block_size = 1; start_idx + block_size - 1 < n - 1; block_size++){
            end_idx = start_idx + block_size - 1;
            ...
            for (int extraction_extent = start_idx+block_size+1; extraction_extent <= n; extraction_extent++){
                //add auxilliary statics on either side, without touching the base sum for the blocksize context
                ...
                if(start_idx > 0){
                    ...
                    // forward
                    element_wise_add(get_waveform(channel, STATIC, 0, start_idx, 0),
                        temp_forward);
                    // backward
                    element_wise_add(get_waveform(channel, STATIC, reverse_sidx + 1, 
                        n - (reverse_sidx + 1), 0), temp_backward);
                }

                // after disp. block
                if(end_idx < extraction_extent-2){    
                    // forward
                    element_wise_add(get_waveform(channel, STATIC, end_idx + 2, extraction_extent - (end_idx + 2), 0), temp_forward); 
                    // backward
                    element_wise_add(get_waveform(channel, STATIC, n-extraction_extent, 1 + (reverse_eidx - 2) - (n-extraction_extent), 0), temp_backward);
                } 

                //storing
                _waveform_hashmap.emplace(get_key(channel, FORWARD, start_idx, block_size, extraction_extent),
                    std::move(temp_forward));

                // std::cout << reverse_eidx-1 << " " << block_size << " " << extraction_extent << std::endl;
                _waveform_hashmap.emplace(get_key(channel, BACKWARD, reverse_eidx-1, block_size, extraction_extent),
...
void Synthesis::WaveformRepo::generate_transfer_waveforms(Channel channel, std::vector<WP> params, std::size_t n) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        _waveform_hashmap.emplace(get_key(channel, EXTRACT, i, 1, 0), 
            edwf);
        temp_waveforms_extract[i] = std::move(edwf);

        _waveform_hashmap.emplace(get_key(channel, IMPLANT, i, 1, 0), 
            idwf);
    ...
    for(int start_idx = 0; start_idx < n; start_idx++){
        ...
        for(int block_size = 2; start_idx + block_size - 1 < n; block_size++){
            ...
            _waveform_hashmap.emplace(get_key(channel, EXTRACT, start_idx, block_size, 0), 
                base_extract);
            _waveform_hashmap.emplace(get_key(channel, IMPLANT, start_idx, block_size, 0), 
                base_implant); 
        }
    }
}
