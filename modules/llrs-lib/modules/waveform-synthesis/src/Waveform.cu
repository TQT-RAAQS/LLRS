...
double Synthesis::Extraction::wave_func(double time) {
    std::make_tuple<double, double, double> srcTuples(0, std::get<1>(params), std::get<2>(params));

    return Synthesis::Waveform::transMod->transition_func(t, srcTuples, params);
}

double Synthesis::Implantation::wave_func(double time) {
    std::make_tuple<double, double, double> destParams(0, std::get<1>(params), std::get<2>(params));

    return Synthesis::Waveform::transMod->transition_func(t, params, destParams);
}
...
double ERF::transition_func(double t, WP params1, WP params2) {
    ...
    double alpha = alpha_mean + dalpha / 2 * erf(
        sqrt(M_PI) * vmax * (t - duration / 2);
    )

    double nu_mean = (nu1 + nu2) / 2;
    double dnu = nu2 - nu1;
    double phi_tilde = nu_mean*t + dnu / (2 * sqrt(M_PI) * vmax) * (
        sqrt(M_PI) * vmax * (t - duration / 2) * erf(sqrt(M_PI) * vmax * (t - duration / 2)) + exp(-pow(sqrt(M_PI)*vmax*(t - duration / 2), 2)) / sqrt(M_PI) -
        sqrt(M_PI) * vmax * duration / 2 * erf(sqrt(M_PI) * vmax * duration / 2) - exp(-pow(sqrt(M_PI)*vmax*(duration / 2), 2)) / sqrt(M_PI)
    )
...
double TANH::transition_func(double t, WP params1, WP params2) {
    ...
    double alpha_mean = (alpha1 + alpha2) / 2;
    double dalpha = alpha2 - alpha_1;
    double alpha = alpha_mean + dalpha / 2 * tanh(
        2 * vmax * (t - duration / 2);
    )

    double nu_mean = (nu1 + nu2) / 2;
    double dnu = nu2 - nu1;
    double phi_tilde = nu_mean*t + dnu / (4 * vmax) * (
...
void read_waveform_configs(std::string filepath) {
	...
	if (transition_type == "TANH") {
		Waveform::set_transition_function(std::make_unique<TANH>(node["transition_type"]));	
	} else if (transition_type == "Spline") {
		Waveform::set_transition_function(std::make_unique<Spline>(node["transition_type"]));	
	} else if (transition_type == "Step") {
		Waveform::set_transition_function(std::make_unique<Step>(node["transition_type"]));	
	} else if (transition_type == "ERF") {
		Waveform::set_transition_function(std::make_unique<ERF>(node["transition_type"]));	
	} else {
	...
	if (transition_type == "sin") {
		Waveform::set_static_function(std::make_unique<Sin>(node["static_type"]));	
	} else {
