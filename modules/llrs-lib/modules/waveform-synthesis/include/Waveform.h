...
enum Modulation { STEP, TANH, ERF };

/**
 * @brief Calculates a modulation function based on the specified modulation
 * type.
 ...
 * @param duration
 */
double transition_func(Modulation mod_type, double time, double relative_slope,
                       double duration);

/**
 * @brief Calculates the definite integral of a modulation function over the
 * range [a,b] based on the modulation type
 * @param integrand
...
class Waveform {
  ...
  public:
    ...
    /**
     * @brief Generates a waveform by evaluating a given wavefunciton for a
     * specified number of samples at a given sample rate
     *  @param num_samples
     *  @param sample_rate
     */
...
class Extraction : public Waveform {
  ...
  public:
     ...
     * parameters obtained from get_params(), and includes amplitude modulation
     * @param index  => index to generate the waveform at
     * @param sample_rate => sample rate of the waveform
     * @return Waveform
     */
...
class Displacement : public Waveform {
  ...
  public:
    ...
    /**
     * @brief generates a displacement waveform sample at a specified index
     * using parameters obtained from get_params(), and includes amplitude
