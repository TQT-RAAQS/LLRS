#ifndef WAVEFORMTABLE_H_
#define WAVEFORMTABLE_H_
 ...
 * The reason for having duplicate enums for 1d and 2d is made purely out of
 * caution and for the sake of readibility.
 */
enum WfMoveType {
    IDLE_1D,     // 0
    IMPLANT_1D,  // 1
...
/**
 * @class WaveformTable
 * @brief the WaveformTable class interleaves waveforms from a WaveformRepo and
 * puts them in arrays to be used when translating RedRec moves to waveforms
 */
class WaveformTable {
  ...
  public:
     ...
     */
    short *get_waveform_ptr(WfMoveType move, int extraction_extent, int index,
                            int offset, int block_size);

    /// Only used in testing

    /**
     * @brief sample wise interleave: even samples are for CHAN0 and odd are for
     * CHAN1
     * @return interleaved wf double the size of the repo's waveform length
     */
    std::vector<short> interleave_waveforms(std::vector<short> even_wf,
                                            std::vector<short> odd_wf);
     ...
     */
    size_t get_blocked_addr(WfType wf_type, size_t index, size_t block_size,
                            size_t max);

    /**
     * @brief Addressing into any primary ingredients
     */
    short *get_primary_wf_ptr(WfType wf_type, size_t index, size_t block_size,
