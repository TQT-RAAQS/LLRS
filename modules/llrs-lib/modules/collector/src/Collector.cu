...
void Util::Collector::get_external_time(const std::string &module, float time) {
    ...
    timers[module] = std::make_pair(time_now, time_now_plus_alg_elapse);
}
...
long long Util::Collector::elapsed_time(const std::string &module) {
    ...
    return std::chrono::duration_cast<std::chrono::nanoseconds>(
               cycle_pair.second - cycle_pair.first)
        .count();
}

std::vector<std::tuple<std::string, long long>>
Util::Collector::get_runtime_data() {
    std::vector<std::tuple<std::string, long long>> runtime_data;
    runtime_data.reserve(NUM_TIMERS);
    for (const auto &timer : timers) {
        runtime_data.push_back(
            std::make_tuple(timer.first, elapsed_time(timer.first)));
    }
    return runtime_data;
}